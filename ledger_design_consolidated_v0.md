...
4. ~~**Cost method:** standard, WAC, or hybrid?~~ **Resolved (Phase 0, 2026-04-27, bd `acct-93b.4`):** `standard` cost only in Phase 0. Schema includes a `cost_method` enum (`'standard' | 'wac' | 'fifo' | 'lot'`) and `skus.cost_method` column (default `'standard'`) so `post_transfers` dispatches on it; non-`standard` branches raise `P0006 cost_method_not_implemented`. WAC / FIFO / lot tracked as `acct-8gg` for future phases. Rationale: smallest correctness surface for Phase 0 (no read-then-write under FOR UPDATE on the std path; deterministic conformance fixtures).
...
1. **Resolve the gating questions** in Part VII items (1) and (2) explicitly — these decide whether v0.2 is right framing.
2. **If v0.2 proceeds:** start Phase 0 (Part IV §13). Estimated 3 weeks.
3. **If v0.1 proceeds instead:** at minimum, fix B1, B2, B3 before code; re-baseline Phase 0 timeline to 6–10 weeks (M1); promote gating §△ items (§△-K, §△-15, §△-10, §△-M, §△-4, §△-H) to v0.2 decisions; add a Type Choice Rationale section (M2, M3); walk the cutover (M4) with the API team.
4. **In either case:** add a conformance test fixture for the write function (`(input, expected_state, expected_response)` triples). Under v0.1 this enforces TB parity; under v0.2 it enforces correctness against a known-good reference implementation.
