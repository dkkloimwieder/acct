...
- 39 sequential reversible migrations under `db/migrations/` (`0001_enable_extensions` through `0039_wo_idempotency_race_fix`). Schema, `post_transfers` (extracted helpers), `reserve_inventory`, `run_daily_reconciliation`, the `_post_transfers_compute_amount` cost dispatcher with real `'standard'` / `'wac_perpetual'` / `'wac_periodic'` / `'wac_retroactive'` branches (`'fifo'`/`'lot'` raise `P0006`); document-layer wrappers (`post_inventory_adjustment`, `post_cost_adjustment`, `post_standard_cost_roll`, `post_cost_adjustment_retroactive`, `post_po_receipt`, `post_ap_bill`, `post_wo_start`, `post_op_move`, `post_wo_complete`, `post_scrap`); period-close orchestration (`close_period` + three real hook bodies); `pg_cron` reservation expiry; the optional `ledger_outbox` table.
- 46 integration test binaries under `tests/`: per-invariant probes (T1 — including the Slice B `slice_b_tables_t1.rs` for `work_orders` / `wo_routings` / `wo_routing_burdens` / `boms` / `wo_events`), workflow matrices (T2 — wac_periodic, wac_retroactive, cost_adjust, cost_adjust_retroactive, standard_cost, period_close, po_receipt, ap_bill, wo_lifecycle), the conformance harness (T5, 129 cases including 8 A16 WO-reason cases), the WAC integration suite, and the perf load matrix (shapes A-M + load_inflow_workload, 9 `#[ignore]`'d). 271 tests pass.
...
- **Slice B WO MVP requires `parent_sku.cost_method = 'standard'`** — Slice B kickoff Q-decision, acct-b82, migration 0038. `post_wo_start` raises **P0026** if the WO parent uses `wac_perpetual` / `wac_periodic` / `wac_retroactive`. WIP-side WAC is structurally compatible (`_post_transfers_compute_amount` `wac_perpetual` branch already handles `inv_value_wip`), but the std-only restriction lets Slice B ship with one cost path and one set of variance-account semantics. Phase 2 Epic J (`acct-p7v`) lifts the restriction.
- **WO burdens are per-op, declared in `wo_routing_burdens`** — Slice B kickoff Q5, acct-b82, migration 0037. BOM components and per-op burdens are isomorphic concepts: per-unit costs that apply as a unit moves through the routing, differing only in *what* (RM substance vs absorption: labor / OH / outside-processing / setup / tooling / energy / …) and *when* (RM at WO_start; burdens at the op they're declared on). `wo_routing_burdens(wo_id, routing_op, applied_account_kind, std_amount)` is the open extension point — adding a new burden type is `ALTER TYPE account_kind ADD VALUE '<X>_applied'` + `ALTER TYPE transfer_reason ADD VALUE '<X>_apply'` + extending `_wo_apply_reason_for(account_kind)` + scaffolding the per-currency `<X>_applied` account. Schema does not change. MVP supports `labor_applied` and `oh_applied`; the rest accrete as needs are real.
...
- **WO BOMs are formal `boms` reference data** — Slice B kickoff Q2, acct-b82. `boms(parent_sku_id, component_sku_id, component_loc_id, qty_per_parent)` is the BOM table. `post_wo_start` expands BOM rows to emit one `rm_issue_to_wo` qty + value pair per component (P0029 if parent has no BOM rows). Phase 2 deferrals: revisions, effectivity windows, alternates, phantom assemblies. Single-level only — sub-assembly composition is a separate WO chained by a parent WO consuming the sub-assembly's output.
- **WO variance grain is residual-only at `wo_complete`** — Slice B kickoff Q3, acct-b82. The MVP surfaces `wo_close_v` against `variance_wo_close` for any nonzero `inv_value_wip@last_op` residual at WO close. Per-op MUV/LV/OHV (operation-level material/labor/overhead variance) is a Phase 2 reporting refinement, deliberately deferred. `variance_muv` / `variance_lv` / `variance_ohv` enum values exist on `account_kind` for forward compatibility but no current code path emits them.
- **`op_move`'s value-leg uses `op_move_v` reason to bypass dispatcher auto-pricing** — acct-b82, migration 0037. The dispatcher's `standard` branch returns `qty × resolve_standard_cost_at(parent_sku, business_date)` which is parent's full std cost — correct only at the last op. `post_op_move` computes `qty × std_cum_at_from_op` externally and uses reason `op_move_v` (not in the dispatcher's cost-event list), so the caller-supplied amount stands. Mirrors the `scrap` / `scrap_v` qty-leg / value-leg split. The fix could alternatively extend the dispatcher's standard branch to read pool unit cost on `inv_value_wip` (mirrors `wac_perpetual` logic), but that's deferred to `acct-p7v` which lifts the WIP-side WAC restriction in general.
