...
- **Period close is orchestrated, not a manual `UPDATE`.** `close_period(p_period_id, p_actor, p_force_provisional, p_force_recon)` (migration 0026, `acct-s6n`) runs hooks, gates, then stamps `closed_at`. Two gates: P0015 (un-finalized `transfers_provisional` rows) and P0016 (recon alerts), each force-flag-bypassable independently. P0014 covers missing/already-closed (and concurrent-caller serialization via `FOR UPDATE` on the period row). As of `acct-9tw` (migration 0031), `wac_periodic_close_hook` (real body from `acct-qfj`) and `wac_retroactive_close_hook` (real body from `acct-9tw`) both take `(p_period_id BIGINT, p_force_provisional BOOLEAN)`. `cost_adjust_retroactive_hook` remains the s6n stub (1-arg, returns 0) pending Epic E (`acct-og1`). wac_retroactive uses chronological replay ordered by `(business_date, posted_at, id)` per pool; WIP class deferred under `acct-p7v` (Phase 2 Epic J). `p_actor` is unvalidated — RBAC = Part VII Q6, still open.
