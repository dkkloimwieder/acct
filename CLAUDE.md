...
- **No outbox in Phase 0** (Part VII Q3 resolved, bd `acct-93b.3`). `post_transfers` is called synchronously inside the same Postgres transaction as document writes. Outbox-vs-sync was characterized in `perf_baseline_v0.md` on 2026-04-29 across **two** drainer architectures: shape G (`acct-tyq`, naive per-row drain) is **9× slower** than direct sync (140 evps committed vs F's 1 274); shape J (`acct-hbg`, super-batched drain merging events from up to 1 000 rows into one `post_transfers` call) is **3.5× slower** (365 evps). Both have caller p99 60-76× lower than F (writers don't queue on the ledger lock — G p99=133 ms, J p99=108 ms), but end-to-end caller wait is 186-217 s p50 because the queue grows unbounded. **D3 stands** — outbox is the right architecture only when caller p99 is the binding constraint AND eventual semantics on the ledger are acceptable; for sync-required ERP flows (invoice, payment), F is dominant by every measure. The remaining gap from J to F is **parallelism, not amortization** (single drainer vs 100 parallel writers); closing it requires multi-worker drainers (`acct-dtv`) or lower per-event `post_transfers` cost (`acct-0ig`).
