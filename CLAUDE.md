...
- `db/fixtures/small/seed.sql` — minimal-but-realistic seed for `cargo test`. Includes un-partitioned `ap_unsettled` / `supplier_pool` / `variance_ppv` for conformance harness; per-supplier scaffold (specific `suppliers` rows, partitioned ap accounts) is created inline by Slice A matrix tests.
