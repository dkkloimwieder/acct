//! Smoke test: spawn the sampler for ~1s against poc_v3_1, assert it captured
//! several ticks without errors. Ignored by default (needs a running poc_v3_1).
...
#[tokio::test]
#[ignore = "needs running poc_v3_1"]
async fn sampler_captures_ticks() {
    let pool = PgPoolOptions::new()
        .max_connections(2)
        .acquire_timeout(Duration::from_secs(5))
        .connect("postgres://acct:acct_dev@localhost:5111/poc_v3_1")
        .await
        .expect("connect");
