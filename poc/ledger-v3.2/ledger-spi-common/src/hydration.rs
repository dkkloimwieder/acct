//! Snapshot hydration for the ledger-core dispatch paths (STD / specific).
//!
//! Reads, for the touched pool set, everything ledger-core needs:
//!   1. pool routing — `method`, `(sku_id, location_id)`.
//!   2. aggregate row (`layer_id = 0`) for every pool — one row per pool.
...
//!      set contains an STD pool. An STD pool with no matching standard_cost row
//!      simply has no entry in `standard_cost_of`; ledger-core raises
//!      `MissingStandardCost` when (and only when) it tries to use it.
//!   5. posting accounts — every touched pool (§3.7), same join shape as
//!      standard_cost. A pool with no posting_account_map row gets no entry;
//!      ledger-core raises `MissingPostingAccounts` when it processes the line.
//!
//! Caller must already hold the pool-row locks (`pool_row_lock::lock_pool_rows`)
//! on every pool_id — otherwise reads can race a concurrent writer and produce a
//! stale snapshot.

use ledger_core::{PoolMethod, PoolStateRow, PostingAccounts, Snapshot};
use pgrx::prelude::*;
...
pub fn hydrate_snapshot(pool_ids: &[i64]) -> Result<Snapshot, pgrx::spi::Error> {
    ...
    Spi::connect(|client| -> Result<(), pgrx::spi::Error> {
        let mut t = client.select(
            "SELECT id, method::text, sku_id, location_id \
               FROM pool \
        ...
        while let Some(row) = t.next() {
            ...
            let sku_id: i64 = row.get::<i64>(3)?.unwrap_or(0);
            let location_id: i64 = row.get::<i64>(4)?.unwrap_or(0);
    ...
    // ── 4. standard_cost (STD pools only) ───────────────────────────
    let needs_standard: Vec<i64> = ids
        .iter()
        .copied()
        .filter(|pid| snapshot.method_of.get(pid).copied() == Some(PoolMethod::Std))
        .collect();
