...
/// Running-average receipt on the aggregate row (§3.1). `line.qty > 0` assumed
/// (caller dispatched on sign).
...
/// Aggregate depletion at `applied_unit_cost` — the running average WAC passes
/// in (§3.1). `line.qty < 0` assumed. unit_cost on the aggregate is unchanged
...
pub(crate) fn aggregate_deplete(
    ...
    //
    // value_sum may legitimately dip NEGATIVE at ±1 scale while qty > 0:
    // banker-rounded running-average depletions post the rounded amount, which
    // can exceed the remaining book value by the rounding residual. The GL
    // stays the truth; the schema permits a negative aggregate value_sum
    // (migration 0003).
