...
| (c) cadence-vs-load control + quiet-backlog mitigation | **acct-q1oj.3** (P1) | The three §19 levers: the §16 configurable recalc-cadence knob (continuous ↔ at-close; real-time = small-period limit), per-pool parallelism interaction, hot-path backpressure on slot-lag (the deliberate re-coupling). Detection: `confirmed_flush_lsn` lag + un-costed-tail count/value + conservation sweep (acct-0at4.5). **Inherits from (b): D8** (cursor advance-on-completion = §17 one-gauge, WAL pins to slowest pool / vs advance-on-ingestion + durable dirty-set = WAL-safe, two gauges — recommend the latter, owns any §17 operational-note update) **+ D11** (historical checkpoint retention depth). |
