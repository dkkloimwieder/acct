...
| child | issue | scope |
|-------|-------|-------|
| (a) layer-walk + chronological re-sort (R-1) | **acct-q1oj.1** (P1) | Per-pool strict FIFO/LIFO layer math + variance vs the recorded provisional; the R-1 `(pool_id, posted_at, id)` re-sort; R-2 backdated re-cost bounding; the real `fifo.rs`/`lifo.rs` that §8 left as `MethodMismatch` stubs. Validated against acct-0at4.7. **Foundational** — b/c/d/e build on it. |
| (b) cross-pool scheduler + parallelization / materialization | **acct-q1oj.2** (P2) | Cross-pool work distribution (pools are independent → parallelize across, serial within); Pareto-hot-pool floor; the moving-tail catch-up loop vs the §6 cursor; what recalc materializes (layer rows, running position, GL value legs, cost-adjustment trx/trx_line); the `trx_line(pool_id, id)` access path (migration 0010). |
| (c) cadence-vs-load control + quiet-backlog mitigation | **acct-q1oj.3** (P1) | The three §19 levers: the §16 configurable recalc-cadence knob (continuous ↔ at-close; real-time = small-period limit), per-pool parallelism interaction, hot-path backpressure on slot-lag (the deliberate re-coupling). Detection: `confirmed_flush_lsn` lag + un-costed-tail count/value + conservation sweep (acct-0at4.5). |
| (d) recalc state/bookkeeping schema + idempotent re-run | **acct-q1oj.4** (P2) | The §7 schema-add surface; full-recompute idempotency (crash-recovery, backdated re-cost, forced re-close) with no double-posting; depletion↔receipt-layer audit linkage. Progress cursor is the §17 slot — **not** a home-grown watermark. |
