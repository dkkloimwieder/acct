...
**[Q-A] Pool-lock granularity.** PoC sketches a `poc_pool_locks` table for committer-to-committer serialization on the same pool. Real cost: one extra SPI per batch. Alternative: rely on row locks via FOR UPDATE on the lock row in a dedicated `poc_pool_lock_anchors` table, populated once per pool. Either works; measure both during M3/M4.
