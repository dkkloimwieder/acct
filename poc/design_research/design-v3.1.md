...
Determinism: for a fixed enqueue order, the committer's behavior is deterministic. Re-running the same commit_group's submissions in the same order produces identical outputs. This is sufficient for the PoC's correctness criteria — repeat-runs of the same workload must produce identical pool_state.aggregate.qty values.
