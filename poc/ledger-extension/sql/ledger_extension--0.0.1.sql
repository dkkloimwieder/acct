/* <begin connected objects> */
/*
This file is auto generated by pgrx.

The ordering of items is not stable, it is driven by a dependency graph.
*/
/* </end connected objects> */

/* <begin connected objects> */
-- src/lib.rs:1987
-- ledger_extension::fifo_apply_batch_inline
CREATE  FUNCTION "fifo_apply_batch_inline"(
	"envelopes" jsonb /* pgrx::datum::json::JsonB */
) RETURNS TABLE (
	"envelope_idx" INT,  /* i32 */
	"status" TEXT,  /* alloc::string::String */
	"posting_line_id" bigint,  /* core::option::Option<i64> */
	"error_code" TEXT,  /* core::option::Option<alloc::string::String> */
	"error_message" TEXT  /* core::option::Option<alloc::string::String> */
)
STRICT
LANGUAGE c /* Rust */
AS 'MODULE_PATHNAME', 'fifo_apply_batch_inline_wrapper';
/* </end connected objects> */

/* <begin connected objects> */
-- src/fifo.rs:1304
-- ledger_extension::fifo::fifo_apply_batch_maximal
CREATE  FUNCTION "fifo_apply_batch_maximal"(
	"envelopes" jsonb /* pgrx::datum::json::JsonB */
) RETURNS TABLE (
	"envelope_idx" INT,  /* i32 */
	"status" TEXT,  /* alloc::string::String */
	"posting_line_id" bigint,  /* core::option::Option<i64> */
	"error_code" TEXT,  /* core::option::Option<alloc::string::String> */
	"error_message" TEXT  /* core::option::Option<alloc::string::String> */
)
STRICT
LANGUAGE c /* Rust */
AS 'MODULE_PATHNAME', 'fifo_apply_batch_maximal_wrapper';
/* </end connected objects> */

/* <begin connected objects> */
-- src/fifo.rs:488
-- ledger_extension::fifo::fifo_arena_capacity
CREATE  FUNCTION "fifo_arena_capacity"() RETURNS bigint /* i64 */
STRICT
LANGUAGE c /* Rust */
AS 'MODULE_PATHNAME', 'fifo_arena_capacity_wrapper';
/* </end connected objects> */

/* <begin connected objects> */
-- src/fifo.rs:2836
-- ledger_extension::fifo::fifo_arena_recon
CREATE  FUNCTION "fifo_arena_recon"() RETURNS TABLE (
	"pool_account_id" bigint,  /* i64 */
	"shmem_live_qty" bigint,  /* i64 */
	"shmem_pending_qty" bigint,  /* i64 */
	"shmem_total_qty" bigint,  /* i64 */
	"shmem_spilled_qty" bigint,  /* i64 */
	"durable_qty" bigint,  /* core::option::Option<i64> */
	"drift" bigint  /* core::option::Option<i64> */
)
STRICT
LANGUAGE c /* Rust */
AS 'MODULE_PATHNAME', 'fifo_arena_recon_wrapper';
/* </end connected objects> */

/* <begin connected objects> */
-- src/fifo.rs:2642
-- ledger_extension::fifo::fifo_arena_reset
CREATE  FUNCTION "fifo_arena_reset"() RETURNS bool /* bool */
STRICT
LANGUAGE c /* Rust */
AS 'MODULE_PATHNAME', 'fifo_arena_reset_wrapper';
/* </end connected objects> */

/* <begin connected objects> */
-- src/fifo.rs:2622
-- ledger_extension::fifo::fifo_drain_consecutive_failures
CREATE  FUNCTION "fifo_drain_consecutive_failures"() RETURNS bigint /* i64 */
STRICT
LANGUAGE c /* Rust */
AS 'MODULE_PATHNAME', 'fifo_drain_consecutive_failures_wrapper';
/* </end connected objects> */

/* <begin connected objects> */
-- src/fifo.rs:2603
-- ledger_extension::fifo::fifo_drain_rows_total
CREATE  FUNCTION "fifo_drain_rows_total"() RETURNS bigint /* i64 */
STRICT
LANGUAGE c /* Rust */
AS 'MODULE_PATHNAME', 'fifo_drain_rows_total_wrapper';
/* </end connected objects> */

/* <begin connected objects> */
-- src/fifo.rs:2612
-- ledger_extension::fifo::fifo_drain_ticks_total
CREATE  FUNCTION "fifo_drain_ticks_total"() RETURNS bigint /* i64 */
STRICT
LANGUAGE c /* Rust */
AS 'MODULE_PATHNAME', 'fifo_drain_ticks_total_wrapper';
/* </end connected objects> */

/* <begin connected objects> */
-- src/fifo.rs:2716
-- ledger_extension::fifo::fifo_force_arena_full
CREATE  FUNCTION "fifo_force_arena_full"() RETURNS bigint /* i64 */
STRICT
LANGUAGE c /* Rust */
AS 'MODULE_PATHNAME', 'fifo_force_arena_full_wrapper';
/* </end connected objects> */

/* <begin connected objects> */
-- src/fifo.rs:2567
-- ledger_extension::fifo::fifo_force_drain_tick
CREATE  FUNCTION "fifo_force_drain_tick"() RETURNS bool /* bool */
STRICT
LANGUAGE c /* Rust */
AS 'MODULE_PATHNAME', 'fifo_force_drain_tick_wrapper';
/* </end connected objects> */

/* <begin connected objects> */
-- src/fifo.rs:495
-- ledger_extension::fifo::fifo_max_layers
CREATE  FUNCTION "fifo_max_layers"() RETURNS bigint /* i64 */
STRICT
LANGUAGE c /* Rust */
AS 'MODULE_PATHNAME', 'fifo_max_layers_wrapper';
/* </end connected objects> */

/* <begin connected objects> */
-- src/fifo.rs:2676
-- ledger_extension::fifo::fifo_overflow_state
CREATE  FUNCTION "fifo_overflow_state"(
	"pool_account_id" bigint /* i64 */
) RETURNS TABLE (
	"overflow_active" bool,  /* bool */
	"watermark" bigint  /* i64 */
)
STRICT
LANGUAGE c /* Rust */
AS 'MODULE_PATHNAME', 'fifo_overflow_state_wrapper';
/* </end connected objects> */

/* <begin connected objects> */
-- src/fifo.rs:2585
-- ledger_extension::fifo::fifo_pending_drain_n
CREATE  FUNCTION "fifo_pending_drain_n"(
	"idx" bigint /* i64 */
) RETURNS bigint /* i64 */
STRICT
LANGUAGE c /* Rust */
AS 'MODULE_PATHNAME', 'fifo_pending_drain_n_wrapper';
/* </end connected objects> */

/* <begin connected objects> */
-- src/fifo.rs:2748
-- ledger_extension::fifo::fifo_release_sentinels
CREATE  FUNCTION "fifo_release_sentinels"() RETURNS bigint /* i64 */
STRICT
LANGUAGE c /* Rust */
AS 'MODULE_PATHNAME', 'fifo_release_sentinels_wrapper';
/* </end connected objects> */

/* <begin connected objects> */
-- src/fifo.rs:505
-- ledger_extension::fifo::fifo_test_acquire_release
CREATE  FUNCTION "fifo_test_acquire_release"(
	"idx" bigint, /* i64 */
	"mode_excl" bool /* bool */
) RETURNS bool /* bool */
STRICT
LANGUAGE c /* Rust */
AS 'MODULE_PATHNAME', 'fifo_test_acquire_release_wrapper';
/* </end connected objects> */

/* <begin connected objects> */
-- src/fifo.rs:523
-- ledger_extension::fifo::fifo_test_acquire_two_sorted
CREATE  FUNCTION "fifo_test_acquire_two_sorted"(
	"idx_a" bigint, /* i64 */
	"idx_b" bigint /* i64 */
) RETURNS bool /* bool */
STRICT
LANGUAGE c /* Rust */
AS 'MODULE_PATHNAME', 'fifo_test_acquire_two_sorted_wrapper';
/* </end connected objects> */

/* <begin connected objects> */
-- src/fifo.rs:550
-- ledger_extension::fifo::fifo_test_cell_lock_addr
CREATE  FUNCTION "fifo_test_cell_lock_addr"(
	"idx" bigint /* i64 */
) RETURNS bigint /* i64 */
STRICT
LANGUAGE c /* Rust */
AS 'MODULE_PATHNAME', 'fifo_test_cell_lock_addr_wrapper';
/* </end connected objects> */

/* <begin connected objects> */
-- src/h_batch_fifo.rs:62
-- ledger_extension::h_batch_fifo::h_apply_batch_fifo
CREATE  FUNCTION "h_apply_batch_fifo"(
	"envelopes" jsonb /* pgrx::datum::json::JsonB */
) RETURNS void
STRICT
LANGUAGE c /* Rust */
AS 'MODULE_PATHNAME', 'h_apply_batch_fifo_wrapper';
/* </end connected objects> */

/* <begin connected objects> */
-- src/h_arena.rs:122
-- ledger_extension::h_arena::h_apply_delta
CREATE  FUNCTION "h_apply_delta"(
	"group_id" bigint, /* i64 */
	"qty_delta" bigint /* i64 */
) RETURNS void
STRICT
LANGUAGE c /* Rust */
AS 'MODULE_PATHNAME', 'h_apply_delta_wrapper';
/* </end connected objects> */

/* <begin connected objects> */
-- src/h_arena.rs:137
-- ledger_extension::h_arena::h_arena_lookup
CREATE  FUNCTION "h_arena_lookup"(
	"group_id" bigint /* i64 */
) RETURNS bigint /* i64 */
STRICT
LANGUAGE c /* Rust */
AS 'MODULE_PATHNAME', 'h_arena_lookup_wrapper';
/* </end connected objects> */

/* <begin connected objects> */
-- src/h_arena.rs:176
-- ledger_extension::h_arena::h_arena_occupied_count
CREATE  FUNCTION "h_arena_occupied_count"() RETURNS bigint /* i64 */
STRICT
LANGUAGE c /* Rust */
AS 'MODULE_PATHNAME', 'h_arena_occupied_count_wrapper';
/* </end connected objects> */

/* <begin connected objects> */
-- src/h_arena.rs:162
-- ledger_extension::h_arena::h_arena_reset
CREATE  FUNCTION "h_arena_reset"() RETURNS void
STRICT
LANGUAGE c /* Rust */
AS 'MODULE_PATHNAME', 'h_arena_reset_wrapper';
/* </end connected objects> */

/* <begin connected objects> */
-- src/h_layer_arena.rs:210
-- ledger_extension::h_layer_arena::h_layer_arena_occupied_count
CREATE  FUNCTION "h_layer_arena_occupied_count"() RETURNS bigint /* i64 */
STRICT
LANGUAGE c /* Rust */
AS 'MODULE_PATHNAME', 'h_layer_arena_occupied_count_wrapper';
/* </end connected objects> */

/* <begin connected objects> */
-- src/h_layer_arena.rs:198
-- ledger_extension::h_layer_arena::h_layer_arena_reset
CREATE  FUNCTION "h_layer_arena_reset"() RETURNS void
STRICT
LANGUAGE c /* Rust */
AS 'MODULE_PATHNAME', 'h_layer_arena_reset_wrapper';
/* </end connected objects> */

/* <begin connected objects> */
-- src/h_layer_arena.rs:120
-- ledger_extension::h_layer_arena::h_layer_create
CREATE  FUNCTION "h_layer_create"(
	"layer_id" bigint, /* i64 */
	"qty" bigint /* i64 */
) RETURNS void
STRICT
LANGUAGE c /* Rust */
AS 'MODULE_PATHNAME', 'h_layer_create_wrapper';
/* </end connected objects> */

/* <begin connected objects> */
-- src/h_layer_arena.rs:146
-- ledger_extension::h_layer_arena::h_layer_decrement
CREATE  FUNCTION "h_layer_decrement"(
	"layer_id" bigint, /* i64 */
	"requested" bigint /* i64 */
) RETURNS bigint /* i64 */
STRICT
LANGUAGE c /* Rust */
AS 'MODULE_PATHNAME', 'h_layer_decrement_wrapper';
/* </end connected objects> */

/* <begin connected objects> */
-- src/h_layer_arena.rs:181
-- ledger_extension::h_layer_arena::h_layer_lookup
CREATE  FUNCTION "h_layer_lookup"(
	"layer_id" bigint /* i64 */
) RETURNS bigint /* i64 */
STRICT
LANGUAGE c /* Rust */
AS 'MODULE_PATHNAME', 'h_layer_lookup_wrapper';
/* </end connected objects> */

/* <begin connected objects> */
-- src/lib.rs:1227
-- ledger_extension::ledger_apply_balance_delta
CREATE  FUNCTION "ledger_apply_balance_delta"(
	"account_id" bigint, /* i64 */
	"period_id" INT, /* i32 */
	"currency_id" smallint, /* i16 */
	"ledger_kind" smallint, /* i16 */
	"amount_delta" bigint, /* i64 */
	"qty_delta" bigint /* i64 */
) RETURNS bigint /* i64 */
STRICT
LANGUAGE c /* Rust */
AS 'MODULE_PATHNAME', 'ledger_apply_balance_delta_wrapper';
/* </end connected objects> */

/* <begin connected objects> */
-- src/lib.rs:1266
-- ledger_extension::ledger_apply_batch
CREATE  FUNCTION "ledger_apply_batch"(
	"envelopes" jsonb /* pgrx::datum::json::JsonB */
) RETURNS bigint /* i64 */
STRICT
LANGUAGE c /* Rust */
AS 'MODULE_PATHNAME', 'ledger_apply_batch_wrapper';
/* </end connected objects> */

/* <begin connected objects> */
-- src/lib.rs:2551
-- ledger_extension::ledger_balance_lookup
CREATE  FUNCTION "ledger_balance_lookup"(
	"account_id" bigint, /* i64 */
	"period_id" INT, /* i32 */
	"currency_id" smallint, /* i16 */
	"ledger_kind" smallint /* i16 */
) RETURNS TABLE (
	"balance" bigint,  /* core::option::Option<i64> */
	"qty" bigint,  /* core::option::Option<i64> */
	"last_seq" bigint  /* core::option::Option<i64> */
)
STRICT
LANGUAGE c /* Rust */
AS 'MODULE_PATHNAME', 'ledger_balance_lookup_wrapper';
/* </end connected objects> */

/* <begin connected objects> */
-- src/lib.rs:2923
-- requires:
--   ledger_balance_lookup


    CREATE TABLE account_balances_rollup (
        account_id  BIGINT NOT NULL,
        period_id   INT NOT NULL,
        currency_id SMALLINT NOT NULL,
        ledger_kind SMALLINT NOT NULL,
        balance     BIGINT NOT NULL DEFAULT 0,
        qty         BIGINT NOT NULL DEFAULT 0,
        last_seq    BIGINT NOT NULL DEFAULT 0,
        drained_at  TIMESTAMPTZ NOT NULL DEFAULT clock_timestamp(),
        PRIMARY KEY (account_id, period_id, currency_id, ledger_kind)
    );

    CREATE FUNCTION balance(
        p_account_id BIGINT,
        p_period_id INT,
        p_currency_id SMALLINT,
        p_ledger_kind SMALLINT
    ) RETURNS TABLE(balance BIGINT, qty BIGINT, last_seq BIGINT, source TEXT) AS $body$
    DECLARE
        s_balance BIGINT;
        s_qty BIGINT;
        s_last_seq BIGINT;
    BEGIN
        SELECT lookup.balance, lookup.qty, lookup.last_seq
          INTO s_balance, s_qty, s_last_seq
          FROM ledger_balance_lookup(p_account_id, p_period_id, p_currency_id, p_ledger_kind) lookup;

        IF s_balance IS NOT NULL THEN
            balance := s_balance;
            qty := s_qty;
            last_seq := s_last_seq;
            source := 'shmem';
            RETURN NEXT;
            RETURN;
        END IF;

        SELECT r.balance, r.qty, r.last_seq
          INTO balance, qty, last_seq
          FROM account_balances_rollup r
         WHERE r.account_id = p_account_id
           AND r.period_id = p_period_id
           AND r.currency_id = p_currency_id
           AND r.ledger_kind = p_ledger_kind;

        IF FOUND THEN
            source := 'rollup';
            RETURN NEXT;
            RETURN;
        END IF;

        balance := 0;
        qty := 0;
        last_seq := 0;
        source := 'none';
        RETURN NEXT;
    END;
    $body$ LANGUAGE plpgsql STABLE;
/* </end connected objects> */

/* <begin connected objects> */
-- src/lib.rs:1627
-- ledger_extension::ledger_dispatch_fifo_batch
CREATE  FUNCTION "ledger_dispatch_fifo_batch"(
	"envelopes" jsonb /* pgrx::datum::json::JsonB */
) RETURNS TABLE (
	"envelope_idx" INT,  /* i32 */
	"row_kind" TEXT,  /* alloc::string::String */
	"kind" TEXT,  /* core::option::Option<alloc::string::String> */
	"debit_account_id" bigint,  /* core::option::Option<i64> */
	"credit_account_id" bigint,  /* core::option::Option<i64> */
	"amount" bigint,  /* core::option::Option<i64> */
	"qty" bigint,  /* core::option::Option<i64> */
	"unit_cost" bigint,  /* core::option::Option<i64> */
	"layer_sentinel" INT,  /* core::option::Option<i32> */
	"layer_id" bigint,  /* core::option::Option<i64> */
	"depl_qty" bigint,  /* core::option::Option<i64> */
	"depl_cost" bigint  /* core::option::Option<i64> */
)
STRICT
LANGUAGE c /* Rust */
AS 'MODULE_PATHNAME', 'ledger_dispatch_fifo_batch_wrapper';
/* </end connected objects> */

/* <begin connected objects> */
-- src/lib.rs:1381
-- ledger_extension::ledger_dispatch_wac_batch
CREATE  FUNCTION "ledger_dispatch_wac_batch"(
	"envelopes" jsonb /* pgrx::datum::json::JsonB */
) RETURNS TABLE (
	"envelope_idx" INT,  /* i32 */
	"debit_account_id" bigint,  /* i64 */
	"credit_account_id" bigint,  /* i64 */
	"amount" bigint,  /* i64 */
	"qty" bigint  /* core::option::Option<i64> */
)
STRICT
LANGUAGE c /* Rust */
AS 'MODULE_PATHNAME', 'ledger_dispatch_wac_batch_wrapper';
/* </end connected objects> */

/* <begin connected objects> */
-- src/lib.rs:1154
-- ledger_extension::ledger_drain_consecutive_fails
CREATE  FUNCTION "ledger_drain_consecutive_fails"() RETURNS bigint /* i64 */
STRICT
LANGUAGE c /* Rust */
AS 'MODULE_PATHNAME', 'ledger_drain_consecutive_fails_wrapper';
/* </end connected objects> */

/* <begin connected objects> */
-- src/lib.rs:1163
-- ledger_extension::ledger_drain_total_failures
CREATE  FUNCTION "ledger_drain_total_failures"() RETURNS bigint /* i64 */
STRICT
LANGUAGE c /* Rust */
AS 'MODULE_PATHNAME', 'ledger_drain_total_failures_wrapper';
/* </end connected objects> */

/* <begin connected objects> */
-- src/lib.rs:1118
-- ledger_extension::ledger_extension_version
CREATE  FUNCTION "ledger_extension_version"() RETURNS TEXT /* &str */
STRICT
LANGUAGE c /* Rust */
AS 'MODULE_PATHNAME', 'ledger_extension_version_wrapper';
/* </end connected objects> */

/* <begin connected objects> */
-- src/lib.rs:1133
-- ledger_extension::ledger_shmem_apply_seq
CREATE  FUNCTION "ledger_shmem_apply_seq"() RETURNS bigint /* i64 */
STRICT
LANGUAGE c /* Rust */
AS 'MODULE_PATHNAME', 'ledger_shmem_apply_seq_wrapper';
/* </end connected objects> */

/* <begin connected objects> */
-- src/lib.rs:1123
-- ledger_extension::ledger_shmem_capacity
CREATE  FUNCTION "ledger_shmem_capacity"() RETURNS bigint /* i64 */
STRICT
LANGUAGE c /* Rust */
AS 'MODULE_PATHNAME', 'ledger_shmem_capacity_wrapper';
/* </end connected objects> */

/* <begin connected objects> */
-- src/lib.rs:1172
-- ledger_extension::ledger_shmem_dirty_count
CREATE  FUNCTION "ledger_shmem_dirty_count"() RETURNS bigint /* i64 */
STRICT
LANGUAGE c /* Rust */
AS 'MODULE_PATHNAME', 'ledger_shmem_dirty_count_wrapper';
/* </end connected objects> */

/* <begin connected objects> */
-- src/lib.rs:1190
-- ledger_extension::ledger_shmem_drained_count
CREATE  FUNCTION "ledger_shmem_drained_count"() RETURNS bigint /* i64 */
STRICT
LANGUAGE c /* Rust */
AS 'MODULE_PATHNAME', 'ledger_shmem_drained_count_wrapper';
/* </end connected objects> */

/* <begin connected objects> */
-- src/lib.rs:1143
-- ledger_extension::ledger_shmem_insert_failure_count
CREATE  FUNCTION "ledger_shmem_insert_failure_count"() RETURNS bigint /* i64 */
STRICT
LANGUAGE c /* Rust */
AS 'MODULE_PATHNAME', 'ledger_shmem_insert_failure_count_wrapper';
/* </end connected objects> */

/* <begin connected objects> */
-- src/lib.rs:1128
-- ledger_extension::ledger_shmem_occupied
CREATE  FUNCTION "ledger_shmem_occupied"() RETURNS bigint /* i64 */
STRICT
LANGUAGE c /* Rust */
AS 'MODULE_PATHNAME', 'ledger_shmem_occupied_wrapper';
/* </end connected objects> */

/* <begin connected objects> */
-- src/lib.rs:2639
-- ledger_extension::ledger_shmem_recon
CREATE  FUNCTION "ledger_shmem_recon"() RETURNS TABLE (
	"account_id" bigint,  /* i64 */
	"shmem_balance" bigint,  /* i64 */
	"shmem_qty" bigint,  /* i64 */
	"ledger_balance" bigint,  /* core::option::Option<i64> */
	"drift" bigint  /* core::option::Option<i64> */
)
STRICT
LANGUAGE c /* Rust */
AS 'MODULE_PATHNAME', 'ledger_shmem_recon_wrapper';
/* </end connected objects> */

/* <begin connected objects> */
-- src/lib.rs:2887
-- ledger_extension::ledger_shmem_reset
CREATE  FUNCTION "ledger_shmem_reset"() RETURNS void
STRICT
LANGUAGE c /* Rust */
AS 'MODULE_PATHNAME', 'ledger_shmem_reset_wrapper';
/* </end connected objects> */

/* <begin connected objects> */
-- src/lib.rs:2794
-- ledger_extension::ledger_test_panic_after_fetch_add
CREATE  FUNCTION "ledger_test_panic_after_fetch_add"(
	"account_id" bigint, /* i64 */
	"period_id" INT, /* i32 */
	"currency_id" smallint, /* i16 */
	"ledger_kind" smallint, /* i16 */
	"amount_delta" bigint, /* i64 */
	"qty_delta" bigint /* i64 */
) RETURNS bigint /* i64 */
STRICT
LANGUAGE c /* Rust */
AS 'MODULE_PATHNAME', 'ledger_test_panic_after_fetch_add_wrapper';
/* </end connected objects> */

/* <begin connected objects> */
-- src/lib.rs:2824
-- ledger_extension::ledger_test_panic_before_fetch_add
CREATE  FUNCTION "ledger_test_panic_before_fetch_add"(
	"account_id" bigint, /* i64 */
	"period_id" INT, /* i32 */
	"currency_id" smallint, /* i16 */
	"ledger_kind" smallint /* i16 */
) RETURNS bigint /* i64 */
STRICT
LANGUAGE c /* Rust */
AS 'MODULE_PATHNAME', 'ledger_test_panic_before_fetch_add_wrapper';
/* </end connected objects> */

/* <begin connected objects> */
-- src/lib.rs:2843
-- ledger_extension::ledger_test_panic_in_exclusive
CREATE  FUNCTION "ledger_test_panic_in_exclusive"(
	"account_id" bigint, /* i64 */
	"period_id" INT, /* i32 */
	"currency_id" smallint, /* i16 */
	"ledger_kind" smallint /* i16 */
) RETURNS bigint /* i64 */
STRICT
LANGUAGE c /* Rust */
AS 'MODULE_PATHNAME', 'ledger_test_panic_in_exclusive_wrapper';
/* </end connected objects> */

