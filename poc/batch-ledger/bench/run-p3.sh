#!/usr/bin/env bash
# acct-k7c6 (P3 of acct-qdp5 PoC) — batch-size sweep on the simplest post_batch.
#
# Sweeps batch sizes [1, 10, 100, 1000, 8000] with 5×60s replicates per size.
# Captures env metadata once.

set -euo pipefail
cd "$(dirname "$0")/.."

TAG="${BENCH_TAG:-p3_batch}"
RUNS="${BENCH_RUNS:-5}"
...
META="${OUT_BASE}/env.txt"
{
    ...
    echo "=== psql server_version ==="
    psql "$DB_URL" -At -c "SHOW server_version;" 2>/dev/null
    psql "$DB_URL" -c "
        SELECT name, setting FROM pg_settings
         WHERE name IN ('synchronous_commit','fsync','shared_buffers','wal_buffers',
                        'io_method','wal_sync_method','max_wal_size')
...
cargo test --manifest-path "$(pwd)/Cargo.toml" --release --no-run --test bench_p3_batch_simple \
    >/dev/null 2>&1
...
for B in $BATCH_SIZES; do
    ...
    for i in $(seq 1 "$RUNS"); do
        ...
        cargo test --manifest-path "$(pwd)/Cargo.toml" --release \
            --test bench_p3_batch_simple \
            -- --ignored --nocapture --test-threads=1 p3_batch_bench \
            2>&1 | tee "$LOG" | tail -15
        if [ "$i" -lt "$RUNS" ]; then
            sleep "$GAP"
        fi
