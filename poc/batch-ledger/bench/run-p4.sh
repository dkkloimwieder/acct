#!/usr/bin/env bash
# acct-4dg2 (P4 of acct-qdp5 PoC) — WAC perpetual batch-size sweep.

set -euo pipefail
cd "$(dirname "$0")/.."

TAG="${BENCH_TAG:-p4_wac}"
RUNS="${BENCH_RUNS:-5}"
...
cargo test --manifest-path "$(pwd)/Cargo.toml" --release --no-run --test bench_p4_wac \
    >/dev/null 2>&1
...
for B in $BATCH_SIZES; do
    ...
    for i in $(seq 1 "$RUNS"); do
        ...
        cargo test --manifest-path "$(pwd)/Cargo.toml" --release \
            --test bench_p4_wac \
            -- --ignored --nocapture --test-threads=1 p4_wac_bench \
            2>&1 | tee "$LOG" | tail -15
