//! P3 append-only — TigerBeetle-aligned batch path, no UPDATE accounts.
//!
//! Mirrors bench_p3_batch_simple.rs but calls post_batch_append_only instead.
//! Used to re-baseline the 80K headline against the tuned PG conf (acct-1vo7).
...
async fn p3_append_only_bench() {
    ...
    eprintln!(
        "P3 append-only bench: workers={workers}, accounts={accounts}, duration={duration_secs}s, batch_size={batch_size}"
    );
    ...
    for wi in 0..workers {
        ...
        handles.push(tokio::spawn(async move {
            ...
            while Instant::now() < deadline {
                ...
                let t0 = Instant::now();
                let res = sqlx::query("SELECT * FROM post_batch_append_only($1)")
    ...
    eprintln!("\n========= P3 append-only bench (batch_size={batch_size}) =========");
