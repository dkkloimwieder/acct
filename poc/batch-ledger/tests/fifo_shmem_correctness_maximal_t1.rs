//! acct-m6q3 — post_batch_fifo_maximal correctness tests.
//!
//! Mirrors wac_shmem_correctness_maximal_t1.rs but routes through
//! `post_batch_fifo_maximal` (mig 0021). The maximal variant pushes the
//! per-pool FIFO layer walk + depletion dispatch fully into Rust via
//! `ledger_dispatch_fifo_batch`. Behavioural contract:
//!
//! - Same envelope shape (transfer / fifo_receipt / fifo_issue).
//! - Same per-envelope status return shape.
//! - Same idempotency-replay semantics (replay returns pre-existing pl id).
//! - In-batch new layers + pre-existing layers consumed in FIFO order.
//! - One cost_layer_depletions row per consumed layer per issue.
//! - SUM(cost_layer_depletions.cost_amount per issue) ==
//!   posting_lines.amount on the issue posting_line.
//!
//! Like the WAC test: replays are pre-filtered before the dispatcher
//! runs, so replay envelopes do NOT contribute to in-batch layer state.
...
async fn t1_receipt_then_issue_lands_correctly() {
    ...
    sqlx::query("SELECT envelope_idx FROM post_batch_fifo_maximal($1::jsonb)")
    ...
    let res = sqlx::query(
        "SELECT envelope_idx, posting_line_id \
         FROM post_batch_fifo_maximal($1::jsonb)",
    )
    .bind(serde_json::json!([{
        "envelope_idx": 0,
        "kind": "fifo_issue",
...
async fn t3_in_batch_sentinel_resolution() {
    ...
    let res = sqlx::query(
        "SELECT envelope_idx, posting_line_id, status \
         FROM post_batch_fifo_maximal($1::jsonb) ORDER BY envelope_idx",
    )
    .bind(envelopes)
    .fetch_all(&p)
    .await
...
async fn t4_idempotent_replay() {
    ...
    let res = sqlx::query("SELECT envelope_idx, status FROM post_batch_fifo_maximal($1::jsonb)")
        .bind(&envelopes)
        .fetch_all(&p)
        .await
        .expect("replay");
    ...
    for _ in 0..writers {
        ...
        handles.push(tokio::spawn(async move {
            ...
            for _ in 0..envelopes_per_writer {
                ...
                let _ = sqlx::query(
                    "SELECT envelope_idx FROM post_batch_fifo_maximal($1::jsonb)",
                )
                .bind(envelopes)
                .execute(&local_pool)
                .await;
