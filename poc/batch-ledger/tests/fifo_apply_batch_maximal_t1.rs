//! acct-uy4p — fifo_apply_batch_maximal correctness tests.
//!
//! Routes through post_batch_fifo_maximal_F (mig 0024; Rust pg_extern
//! fifo_apply_batch_maximal). Mirrors fifo_shmem_correctness_maximal_t1 so
...
fn unique_accounts() -> (i64, i64, i64) {
    ...
    let base = 2_100_000_000_000_i64
        + ((bytes[0] as i64) << 24
            | (bytes[1] as i64) << 16
            | (bytes[2] as i64) << 8
            | (bytes[3] as i64))
...
async fn t1_receipt_then_issue_lands_correctly() {
    ...
    sqlx::query("SELECT envelope_idx FROM post_batch_fifo_maximal_F($1::jsonb)")
...
async fn t3_in_batch_sentinel_resolution() {
    ...
    let res = sqlx::query(
        "SELECT envelope_idx, posting_line_id, status \
         FROM post_batch_fifo_maximal_F($1::jsonb) ORDER BY envelope_idx",
    )
    .bind(envelopes)
    .fetch_all(&p)
    .await
...
async fn t4_idempotent_replay() {
    ...
    let res = sqlx::query("SELECT envelope_idx, status FROM post_batch_fifo_maximal_F($1::jsonb)")
        .bind(&envelopes)
        .fetch_all(&p)
        .await
        .expect("replay");
...
async fn t5_multi_writer_fan_in_coupled_writes() {
    ...
    let writers = 6;
    let envelopes_per_writer: i64 = 8;
    ...
    for _ in 0..writers {
        ...
        handles.push(tokio::spawn(async move {
            ...
            for _ in 0..envelopes_per_writer {
                ...
                let _ = sqlx::query(
                    "SELECT envelope_idx FROM post_batch_fifo_maximal_F($1::jsonb)",
                )
                .bind(envelopes)
                .execute(&local_pool)
                .await;
