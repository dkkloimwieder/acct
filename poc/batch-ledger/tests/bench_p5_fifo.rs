//! P5 — batch API + FIFO throughput bench.
//!
//! Workload: 30% fifo_receipt + 70% fifo_issue against 20 pools. Pools
//! pre-seeded with 10K qty/layer × multiple layers so workers don't drain them.
...
async fn p5_fifo_bench() {
    ...
    let ap_id: i64 = sqlx::query_scalar(
        "INSERT INTO accounts (code, currency, kind) VALUES ('ap-bench-p5', 'USD', 'credit_normal') RETURNING id"
    ).fetch_one(&pool).await.expect("ap");
    let cogs_id: i64 = sqlx::query_scalar(
        "INSERT INTO accounts (code, currency, kind) VALUES ('cogs-bench-p5', 'USD', 'debit_normal') RETURNING id"
    ...
    for i in 0..pools {
        ...
        ).bind(format!("p5-pool-{i:02}")).fetch_one(&pool).await.expect("pool seed");
        pool_ids.push(id);
    }

    // Pre-seed each pool with several large layers so workers find layers to deplete.
    // 5 layers × 1M qty @ 100 = 5M qty / pool. Enough for a 60s bench at 5K issues/sec.
    for layer_no in 0..5 {
        ...
        sqlx::query("SELECT * FROM post_batch($1)")
            ...
            .execute(&pool).await.expect("layer seed");
    }
    ...
    eprintln!("P5 FIFO bench starting: workers={workers}, pools={pools}, duration={duration_secs}s, batch_size={batch_size}");
    ...
    eprintln!("\n=========== P5 FIFO bench (batch_size={batch_size}) ===========");
