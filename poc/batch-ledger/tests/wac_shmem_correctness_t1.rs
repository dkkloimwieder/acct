//! acct-n4mo / M10.B4 — post_batch_wac_shmem correctness tests.
//!
//! Probes the WAC shmem apply path (mig 0014). Each test uses
//! UUID-derived synthetic account_ids so the M6 lazy-load can't pick
//! up stale `account_balances_rollup` rows from prior runs. Period=1,
//! currency_id=1, ledger_kind=1 matches the PoC recon convention so
//! `ledger_shmem_recon()` surfaces these cells.
//!
//! # Tests
//!
//! - **T1** receipt+issue in two consecutive batches: shmem (balance, qty)
//!   matches the closed-form expectation; recon drift=0.
//! - **T2** cross-batch sequencing: receipt → drain → issue → drain;
//!   second batch reads the updated pool snapshot from shmem and prices
//!   the issue at the correct running avg.
//! - **T3** in-batch running avg: one batch with two `wac_receipt`s at
//!   different unit costs followed by a `wac_issue` — the issue must
//!   use the running average (post-both-receipts), NOT the first or
//!   last receipt's unit_cost in isolation.
//! - **T4** idempotent replay: re-submitting the same idempotency_key
//!   returns `idempotent_replay` and does NOT double-apply to shmem.
//! - **T5** multi-writer fan-in: 8 writers × 50 wac_receipts on a shared
//!   pool. After settle: balance == qty × unit_cost; recon drift=0.
//!
//! Run with `--test-threads=1` so concurrent test binaries don't
//! perturb shmem state for tests that probe recon globally.
...
/// 4 distinct account_ids per test invocation, far from anyone else's
/// synthetic range. UUID-derived; never overlap two runs of this test.
fn unique_accounts() -> (i64, i64, i64, i64) {
    let u = Uuid::new_v4();
    let bytes = u.as_bytes();
    let base = 1_400_000_000_000_i64
        + ((bytes[0] as i64) << 24
            | (bytes[1] as i64) << 16
            | (bytes[2] as i64) << 8
            | (bytes[3] as i64))
...
/// Wait until the bgworker has drained dirty cells.
...
/// T1 — receipt then issue in two batches; pool ends at half balance/qty.
#[tokio::test]
async fn t1_receipt_then_issue_lands_correctly() {
    ...
    seed_accounts(
        &p,
        &[
            (pool_id, "wac_pool", "inv_value_raw"),
            (ap_id, "ap_counter", "credit_normal"),
            (cogs_id, "cogs", "debit_normal"),
        ],
    )
    .await;

    let receipt_idem = Uuid::new_v4();
    let issue_idem = Uuid::new_v4();

    let res = sqlx::query(
        "SELECT envelope_idx, status FROM post_batch_wac_shmem($1::jsonb)",
    )
    .bind(serde_json::json!([{
        ...
        "qty": 100,
        "unit_cost": 1000,
        "idempotency_key": receipt_idem.to_string(),
    ...
    assert_eq!(status, "committed", "T1: receipt status");
    ...
    .bind(serde_json::json!([{
        ...
        "qty": 40,
        "idempotency_key": issue_idem.to_string(),
        "business_date": "2026-05-13",
    }]))
    .execute(&p)
    .await
...
/// T2 — cross-batch sequencing: 3 receipts at varying unit_costs, drain,
/// then 1 issue. Issue prices at the running avg from shmem.
#[tokio::test]
async fn t2_cross_batch_running_avg_via_shmem() {
    ...
    // Three receipt batches with different unit_costs to break any "first-cost" or "last-cost" bug.
    for (qty, uc) in [(100i64, 1000i64), (50, 1200), (50, 800)] {
        sqlx::query("SELECT envelope_idx FROM post_batch_wac_shmem($1::jsonb)")
            .bind(serde_json::json!([{
                "envelope_idx": 0,
                "kind": "wac_receipt",
                "debit_account_id": pool_id,
    ...
    // Total value = 100*1000 + 50*1200 + 50*800 = 100_000 + 60_000 + 40_000 = 200_000
    // Total qty   = 200; running avg = 1000
    let (bal, qty) = lookup(&p, pool_id).await;
    ...
    assert_eq!(qty, Some(120), "T2: pool qty after issue (200 - 80)");

    let (cogs_bal, _) = lookup(&p, cogs_id).await;
    assert_eq!(cogs_bal, Some(80_000), "T2: COGS = issued qty × running_avg = 80 × 1000");

    cleanup(&p, &[pool_id, ap_id, cogs_id]).await;
}

/// T3 — in-batch running avg: 2 receipts + 1 issue in ONE batch.
/// The in-batch map (v_pool_value, v_pool_qty) must update receipt-by-
/// receipt so the issue sees both.
#[tokio::test]
async fn t3_in_batch_running_avg() {
    ...
    // One batch: receipt(100 @ 1000) + receipt(100 @ 1500) + issue(50).
    // After both receipts: value=250_000, qty=200, avg=1250.
    // Issue 50 @ avg 1250 = 62_500. Final pool: 187_500 / 150.
...
/// batch returns `idempotent_replay` and shmem is not double-applied.
#[tokio::test]
...
/// at unit_cost=1000 against ONE shared pool. After all join + drain:
/// - pool balance = 8 × 50 × 1000 × 100 (qty per envelope) per the loop math.
/// - pool qty matches; balance/qty == 1000 (uniform unit_cost).
/// - recon drift = 0.
...
async fn t5_multi_writer_fan_in_coupled_writes() {
    ...
    for _ in 0..writers {
        ...
        handles.push(tokio::spawn(async move {
            ...
            for _ in 0..envelopes_per_writer {
                ...
                let _ = sqlx::query("SELECT envelope_idx FROM post_batch_wac_shmem($1::jsonb)")
                    .bind(envelopes)
                    .execute(&local_pool)
                    .await;
            }
    ...
    assert_eq!(
        qty,
        Some(expected_qty),
        "T5: pool qty must equal writers × envelopes_per_writer × qty_per_envelope"
    );
    assert_eq!(
        bal,
        Some(expected_bal),
        "T5: pool balance must equal qty × unit_cost (uniform receipts)"
    );

    // unit_cost invariant: bal / qty must equal the uniform 1000 every time.
    let unit_check = bal.unwrap() / qty.unwrap();
    assert_eq!(
        unit_check, unit_cost,
        "T5: implied unit_cost from shmem must equal the constant 1000"
    );
