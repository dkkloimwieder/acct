//! Postgres-side lock contention sampler for ledger-v3 measurement runs.
//!
//! Ported from `poc/queue-extension-v21/tests/common/pg_locks_sampler.rs`
//! (M9.2 / acct-4d4n.21). 10 Hz dedicated-connection sampler accumulating
//! per-(relation, locktype, mode, granted) wait counts and per-
//! (wait_event_type, wait_event) backend counts. On shutdown emits a
//! `SamplerReport`.
//!
//! Wired by the `run` subcommand drivers (acct-ykyl direct,
//! acct-qiaz routed). `--no-sampler` on the CLI suppresses spawn for
//! perturbation checks (re-run a cell with sampler off; verify p99
//! sits inside the sampler-on IQR).
//!
//! Setting `LEDGER_V3_PRINT_SAMPLER=1` in the run environment causes
//! the driver to dump `SamplerReport::format()` to stderr at end of run
//! (mirror of v21's `POC_PL3B_PRINT_SAMPLER`).
//!
//! Perturbation: 1 dedicated connection (caller's pool must size for
//! it). Sampling cost at 10 Hz is a small fraction of typical workload
//! CPU.
...
/// Read the `LEDGER_V3_PRINT_SAMPLER` env var. Returns true when set to
/// "1" / "true" (case-insensitive). Drivers consult this at end-of-run
/// to decide whether to dump SamplerReport::format() to stderr.
pub fn print_sampler_enabled() -> bool {
    std::env::var("LEDGER_V3_PRINT_SAMPLER")
        .map(|v| {
