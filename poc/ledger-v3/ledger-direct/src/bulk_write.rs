//! Bulk-write helpers (design-v3 §4.2 step 7).
//!
//! Four entry points, called in FK order by `submit::ledger_submit_trx`:
...
//! v21 relies on this same property in committer.rs (~line 1815).
...
/// 7.1 — INSERT INTO trx. Returns the new trx.id.
///
...
/// 7.2 — Bulk INSERT INTO trx_line ... RETURNING id, in input order.
///
...
/// 7.3-7.6 — Apply pool_state mutations in four batches: Insert, Upsert,
...
/// 7.7 — Bulk INSERT INTO posting_line ... RETURNING id, in input order.
///
/// The PlanResult contract is that `posting_lines[i]` corresponds to the
/// i-th inserted row, so the returned Vec is index-aligned and consumable
/// by `insert_provisional_postings` for wac_periodic flags (acct-s6fa).
///
/// Resolves `trx_line_idx` against the input-order `trx_line_ids` from
/// step 7.2.
#[allow(dead_code)]
pub fn insert_posting_lines(
    ...
    // INSERT INTO ... SELECT ... UNNEST RETURNING preserves input order —
    // PG processes the SELECT rows sequentially and RETURNING follows the
    // physical insert order. Same property `insert_trx_lines` and v21
    // committer.rs (~line 1815) rely on.
...
/// 7.8 — Bulk INSERT INTO posting_lines_provisional (acct-s6fa).
///
...
/// Convenience wrapper: run the full §4.2 step 7 sequence given the
/// orchestrator's trx-level fields and a fresh PlanResult.
#[allow(dead_code)]
