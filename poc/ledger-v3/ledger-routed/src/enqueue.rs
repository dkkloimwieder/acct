//! Path B SPI entry point: `ledger_enqueue_trx` (design-v3 §5.1, §5.4 step 0).
//!
...
fn ledger_enqueue_trx(
    ...
    let posted_at_micros: i64 = match DateTime::parse_from_rfc3339(posted_at) {
        ...
        Err(e) => ereport_invalid_arg(
            ...
            format!("ledger_enqueue_trx: posted_at not RFC3339: {e}"),
    ...
    let line_vec: Vec<PocV3Line> = match serde_json::from_value(lines.0) {
        ...
        Err(e) => ereport_invalid_arg(
            ...
            format!("ledger_enqueue_trx: lines JSONB decode failed: {e}"),
            "Expected an array of {line_type, source_id?, pool_id, qty, unit_cost, debit_account, credit_account}.",
        ),
    };
    if line_vec.is_empty() {
        ereport_invalid_arg(
            PgSqlErrorCode::ERRCODE_INVALID_PARAMETER_VALUE,
            "ledger_enqueue_trx: lines must be non-empty".to_string(),
    ...
    let pool_count: u16 = match u16::try_from(pool_ids.len()) {
        ...
        Err(_) => ereport_invalid_arg(
            ...
            format!(
                "ledger_enqueue_trx: too many distinct pools touched ({} > {})",
                pool_ids.len(),
                u16::MAX
            ),
        ...
        let (payload_offset, payload_length, pool_keys_offset) = match alloc_outcome {
            ...
            AllocOutcome::ArenaFull => {
                if past_deadline(deadline_micros) {
                    ...
                    ereport!(
                        ...
                        format!(
                            "ledger_enqueue_trx: spillover arena exhausted and backpressure timeout elapsed ({timeout_ms}ms)"
                        )
                    );
                }
            ...
            AllocOutcome::Internal(detail) => {
                ...
                ereport_internal(format!(
                    "ledger_enqueue_trx: payload encode failed: {detail}"
                ));
            }
        };
            ...
            Err(PushError::QueueFull) => {
                ...
                if past_deadline(deadline_micros) {
                    ...
                    ereport!(
                        ...
                        PgSqlErrorCode::ERRCODE_INSUFFICIENT_RESOURCES,
                        format!(
                            "ledger_enqueue_trx: staging queue full and backpressure timeout elapsed ({timeout_ms}ms)"
                        )
                    );
                }
                cv_wait_for_slot(cv, deadline_micros);
...
#[pg_extern]
fn ledger_routed_staging_state_counts()
...
#[pg_extern]
fn ledger_routed_staging_request_seq_max() -> i64 {
