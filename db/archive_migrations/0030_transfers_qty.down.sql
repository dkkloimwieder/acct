-- Revert acct-1vr / acct-75z.1.
-- Restores 0029's _post_transfers_compute_amount, post_transfers,
-- post_inventory_adjustment, wac_periodic_close_hook (verbatim);
-- drops transfers.qty column.
--
-- DATA LOSS: dropping qty discards any per-event qty data populated
-- since 0030 was applied. Acceptable — column is forward-only;
-- Phase 0/1 has no production data; dev DB wipes via dev-down.sh.

ALTER TABLE transfers DROP CONSTRAINT IF EXISTS transfers_qty_nonneg;
ALTER TABLE transfers DROP COLUMN IF EXISTS qty;

-- Restore 0029's _post_transfers_compute_amount (no SUM divisor;
-- uses _post_transfers_lookup_qty_account + accounts.balance).
...
-- Restore 0029's post_transfers (no qty column in INSERT).
  ...
  IF NOT v_has_wac THEN
    FOR v_idx IN 1..v_n LOOP
      ...
      IF v_d_acct.ledger_kind <> v_c_acct.ledger_kind THEN
        RAISE EXCEPTION 'ledger_mismatch: event index % (% vs %)', v_idx, v_d_acct.ledger_kind, v_c_acct.ledger_kind
          USING ERRCODE = 'P0002';
      END IF;
      IF v_d_acct.ledger_kind = 'value' AND v_d_acct.currency <> v_c_acct.currency THEN
        RAISE EXCEPTION 'currency_mismatch: event index % (% vs %)', v_idx, v_d_acct.currency, v_c_acct.currency
          USING ERRCODE = 'P0003';
      END IF;
      ...
      INSERT INTO transfers (
        ...
        (v_event->>'document_line_id')::UUID, v_d_id, v_c_id, v_amount,
...
-- Restore 0029's post_inventory_adjustment (no qty in event JSON; uses
-- accounts.balance for pool reads).
...
BEGIN
  ...
  WHEN 'wac_perpetual' THEN
    ...
    SELECT debits_total - credits_total INTO v_qty_balance FROM accounts WHERE id = v_qty_acct;
  ...
  WHEN 'wac_periodic' THEN
    ...
    IF p_qty_delta > 0 THEN
      IF p_unit_cost IS NULL THEN
        IF v_qty_balance <= 0 THEN
          RAISE EXCEPTION 'wac_periodic SKU % at sku=% loc=% has empty pool (qty_balance=%); caller must pass p_unit_cost on first adjustment-in to seed', p_sku_id, p_sku_id, p_location_id, v_qty_balance USING ERRCODE = 'P0011';
        END IF;
        v_effective_uc := v_val_balance / v_qty_balance;
      ELSE
        v_effective_uc := p_unit_cost;
    ...
    ELSE
      IF p_unit_cost IS NOT NULL THEN
        RAISE EXCEPTION 'wac_periodic depletion does not accept asserted unit_cost (got % on sku=% loc=%); use lot cost_method (acct-8gg) for asserted-cost-per-transaction needs', p_unit_cost, p_sku_id, p_location_id USING ERRCODE = 'P0011';
      END IF;
      IF v_qty_balance <= 0 THEN
        RAISE EXCEPTION 'wac_periodic SKU % at sku=% loc=% has empty pool; cannot deplete', p_sku_id, p_sku_id, p_location_id USING ERRCODE = 'P0010';
      END IF;
      v_effective_uc := v_val_balance / v_qty_balance;
  ...
  IF v_val_amount > 0 THEN
    v_batch := jsonb_build_array(
      jsonb_build_object('reason','inventory_adjustment','document_kind','inventory_adjustment','document_id',v_doc_id,'debit_account_id',v_qty_debit,'credit_account_id',v_qty_credit,'amount',v_qty_amount,'business_date',p_business_date,'idempotency_key',gen_random_uuid(),'posted_by',p_posted_by),
      jsonb_build_object('reason','inventory_adjustment','document_kind','inventory_adjustment','document_id',v_doc_id,'debit_account_id',v_val_debit,'credit_account_id',v_val_credit,'amount',v_val_amount,'business_date',p_business_date,'idempotency_key',gen_random_uuid(),'posted_by',p_posted_by)
    );
  ELSE
    v_batch := jsonb_build_array(
      jsonb_build_object('reason','inventory_adjustment','document_kind','inventory_adjustment','document_id',v_doc_id,'debit_account_id',v_qty_debit,'credit_account_id',v_qty_credit,'amount',v_qty_amount,'business_date',p_business_date,'idempotency_key',gen_random_uuid(),'posted_by',p_posted_by)
...
-- Restore 0029's wac_periodic_close_hook (uses _post_transfers_lookup_qty_account
-- + sums against stock_available — pre-fix behavior).
...
DECLARE
  ...
  v_event_b        JSONB;
BEGIN
  SELECT opens_at, closes_at, code INTO v_period_opens, v_period_closes, v_period_code FROM periods WHERE id = p_period_id;
  IF NOT FOUND THEN RETURN 0; END IF;

  FOR v_row IN
    SELECT * FROM transfers_provisional
     WHERE period_id = p_period_id AND cost_method = 'wac_periodic' AND finalized_at IS NULL
  ...
  LOOP
    ...
    SELECT COALESCE(SUM(amount), 0) INTO v_pool_value_in
      FROM transfers WHERE debit_account_id = v_pool_acct.id AND business_date BETWEEN v_period_opens AND v_period_closes;

    DECLARE v_qty_acct_id BIGINT;
    BEGIN
      v_qty_acct_id := _post_transfers_lookup_qty_account(v_pool_acct);
      IF v_qty_acct_id IS NULL THEN
        RAISE EXCEPTION 'wac_periodic_close: cannot resolve qty account for value pool %', v_pool_acct.id USING ERRCODE = 'P0010';
      END IF;
      SELECT COALESCE(SUM(amount), 0) INTO v_pool_qty_in
        FROM transfers WHERE debit_account_id = v_qty_acct_id AND business_date BETWEEN v_period_opens AND v_period_closes;
    END;

    IF v_pool_qty_in = 0 THEN
      IF p_force_provisional THEN CONTINUE; END IF;
      RAISE EXCEPTION 'wac_periodic_close_no_receipts: period % (id=%) has provisional depletions on pool kind=% sku=% loc=% ccy=% but zero receipts in period; post receipts and retry the close, or close with p_force_provisional=TRUE.',
        v_period_code, p_period_id, v_pool_acct.kind, v_pool_acct.sku_id, v_pool_acct.location_id, v_pool_acct.currency USING ERRCODE = 'P0020';
    END IF;
    ...
    IF v_variance = 0 THEN
      UPDATE transfers_provisional SET finalized_at = clock_timestamp(), variance_amount = 0, variance_transfer_id = NULL WHERE transfer_id = v_row.transfer_id;
      v_count := v_count + 1;
      CONTINUE;
    END IF;

    SELECT id INTO v_var_acct FROM accounts WHERE kind = 'variance_wac_period' AND ledger_kind = 'value' AND currency = v_pool_acct.currency AND NOT is_closed;
    IF v_var_acct IS NULL THEN
      RAISE EXCEPTION 'wac_periodic_close: no variance_wac_period(value, ccy=%) account configured', v_pool_acct.currency USING ERRCODE = 'P0010';
    END IF;
    ...
    IF v_variance > 0 THEN
      v_event_a := jsonb_build_object('reason','cost_restate','document_kind','wac_periodic_close','document_id',gen_random_uuid(),'debit_account_id',v_orig_debit,'credit_account_id',v_var_acct,'amount',v_variance,'business_date',v_period_closes,'idempotency_key',gen_random_uuid(),'posted_by','00000000-0000-0000-0000-000000000000');
      v_event_b := jsonb_build_object('reason','cost_restate','document_kind','wac_periodic_close','document_id',gen_random_uuid(),'debit_account_id',v_var_acct,'credit_account_id',v_orig_credit,'amount',v_variance,'business_date',v_period_closes,'idempotency_key',gen_random_uuid(),'posted_by','00000000-0000-0000-0000-000000000000');
    ELSE
      v_event_a := jsonb_build_object('reason','cost_restate','document_kind','wac_periodic_close','document_id',gen_random_uuid(),'debit_account_id',v_var_acct,'credit_account_id',v_orig_debit,'amount',-v_variance,'business_date',v_period_closes,'idempotency_key',gen_random_uuid(),'posted_by','00000000-0000-0000-0000-000000000000');
      v_event_b := jsonb_build_object('reason','cost_restate','document_kind','wac_periodic_close','document_id',gen_random_uuid(),'debit_account_id',v_orig_credit,'credit_account_id',v_var_acct,'amount',-v_variance,'business_date',v_period_closes,'idempotency_key',gen_random_uuid(),'posted_by','00000000-0000-0000-0000-000000000000');
    END IF;
    ...
    UPDATE transfers_provisional SET finalized_at = clock_timestamp(), variance_amount = v_variance, variance_transfer_id = v_var_xfer_id WHERE transfer_id = v_row.transfer_id;
    v_count := v_count + 1;
  END LOOP;
