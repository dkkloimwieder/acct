...
-- pg_cron schedule.
--
-- Two checks per Part IV §7 (the two that Phase 0 has the schema
-- groundwork for):
--
--   1. Per-ledger double-entry: SUM(debits) = SUM(credits) per
--      (ledger_kind, currency). Violations indicate either bypass of
--      post_transfers or in-place corruption — neither should happen
--      in normal operation.
--
--   2. Reservation over-promise: per (sku, location), the
--      stock_available on-hand must be ≥ SUM(qty) of active
--      reservations. reserve_inventory() (migration 0014) prevents
--      this at insert time; this check catches direct INSERTs that
--      bypass the function or any future drift.
--
-- Both checks INSERT a row per detected anomaly into
-- reconciliation_alerts. Phase 0 is log-only — no PagerDuty/Slack
-- integration. The schema is structured so future alerting can hang
-- off (alert_type, payload, created_at) without further migrations.
--
-- Cadence: daily at 00:00 UTC. The table + function are always
-- created (so the recon function is testable in cargo test against
-- acct_test). The cron schedule is wrapped in DO/EXCEPTION so
-- non-acct DBs migrate cleanly (same pattern as 0001 / 0015).
...
CREATE INDEX recon_alerts_created
  ON reconciliation_alerts (created_at DESC);

CREATE INDEX recon_alerts_type_created
  ON reconciliation_alerts (alert_type, created_at DESC);
...
BEGIN
  ...
  INSERT INTO reconciliation_alerts (alert_type, payload)
