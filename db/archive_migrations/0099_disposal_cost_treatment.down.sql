-- acct-6g47 / acct-7t4.4 down: restore mig 0098 post_wo_complete
-- (no disposal_cost handling; disposal_cost rows fall through CONTINUE).
--
-- Note: ALTER TYPE account_kind ADD VALUE entries are NOT removed (PG
-- enum values can't be dropped without rebuilding the type). Per project
-- convention (CLAUDE.md down migrations are best-effort).
