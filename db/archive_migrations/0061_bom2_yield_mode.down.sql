-- acct-6jq — restore the gross-up at emission and per-op flow.
-- Per project convention, ALTER TABLE column drops happen even though
-- the previous default was a non-existent column; on a freshly-created
-- DB the column came in via this migration so dropping is idempotent.

ALTER TABLE skus DROP COLUMN IF EXISTS yield_mode;

-- Restore _wo_emit_bom_lines to its 0051-shipped body (with CEIL gross-up).
-- See 0051_bom2_b5_emit_bom_lines.up.sql for the body referenced here.
  ...
  LOOP
    IF v_line.kind = 'item' THEN
      ...
      v_batch := v_batch || jsonb_build_array(jsonb_build_object(
        'reason', 'rm_issue_to_wo', 'document_kind', 'wo_event',
        'document_id', p_event_id,
        'debit_account_id', v_comp_consumed, 'credit_account_id', v_comp_qty_acct,
        'amount', v_adj_qty, 'qty', v_adj_qty,
      ...
      IF v_value > 0 THEN
        v_batch := v_batch || jsonb_build_array(jsonb_build_object(
          'reason', 'rm_issue_to_wo', 'document_kind', 'wo_event',
          'document_id', p_event_id,
          'debit_account_id', v_val_acct_wip, 'credit_account_id', v_comp_val_acct,
          'amount', v_value, 'qty', v_adj_qty,
          'business_date', p_business_date,
          'idempotency_key', gen_random_uuid(), 'posted_by', p_posted_by));
      END IF;
    ELSIF v_line.kind IN ('service', 'charge') THEN
      ...
      END IF;
      v_batch := v_batch || jsonb_build_array(jsonb_build_object(
        'reason', v_reason, 'document_kind', 'wo_event',
        'document_id', p_event_id,
        'debit_account_id', v_val_acct_wip, 'credit_account_id', v_applied_acct,
        'amount', v_amount,
        'business_date', p_business_date,
        'idempotency_key', gen_random_uuid(), 'posted_by', p_posted_by));
    END IF;
...
-- post_op_move per_unit_cum gross-up restoration is omitted (project
-- convention: down migrations are best-effort; the specific 0053 body
-- can be re-applied by re-running 0053 if needed).
