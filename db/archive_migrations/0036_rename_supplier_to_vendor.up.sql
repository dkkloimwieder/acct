-- acct-397 — Rename supplier → vendor throughout the inflow surface.
--
-- The original Slice A design used 'supplier' (matching the speculative
-- §3.1 PO receipt example in the consolidated doc). User preference is
-- 'vendor' across the codebase. Doing the rename now, before more code
-- accretes 'supplier' references, is cheaper than later.
--
-- Schema renames in this migration:
--   * suppliers (table)                          → vendors
--   * purchase_orders.supplier_id (column + FK)  → vendor_id (po_vendor_fk)
--   * vendor_bills.supplier_id (column + index)  → vendor_id (vendor_bills_vendor)
--   * accounts_supplier_pool_uk (index)          → accounts_vendor_pool_uk
--   * account_kind enum value 'supplier_pool'    → 'vendor_pool'
--
-- Functions rewritten (CREATE OR REPLACE — types unchanged, parameter
-- name and internal vars rename only):
--   * post_po_receipt — p_supplier_id removed (lookup via PO header
--     stays); internal v_supplier_id → v_vendor_id; error messages
--     reword to 'vendor'.
--   * post_ap_bill — p_supplier_id → p_vendor_id; internal vars rename;
--     error messages rewordex.
--
-- Schema digest changes (table/column/index/value/function bodies);
-- ci-check round-trip confirms cleanliness. Forward-only beyond this
-- point: down migration restores 'supplier' verbatim, but pre-0036 dev
-- DBs can't roll forward without re-applying.
--
-- 'customer_pool' (the symmetric counterparty kind) is intentionally
-- left unchanged — different concept, different terminology, and the
-- user's preference was supplier→vendor specifically.
...
ALTER TABLE purchase_orders RENAME COLUMN supplier_id TO vendor_id;
ALTER TABLE purchase_orders RENAME CONSTRAINT po_supplier_fk TO po_vendor_fk;

ALTER TABLE vendor_bills RENAME COLUMN supplier_id TO vendor_id;
ALTER INDEX vendor_bills_supplier RENAME TO vendor_bills_vendor;

ALTER INDEX accounts_supplier_pool_uk RENAME TO accounts_vendor_pool_uk;

ALTER TYPE account_kind RENAME VALUE 'supplier_pool' TO 'vendor_pool';

-- post_ap_bill's first parameter renames p_supplier_id → p_vendor_id.
-- PG forbids renaming function parameters via CREATE OR REPLACE; drop
-- + recreate. post_po_receipt keeps its signature (only internal var
-- names change), so CREATE OR REPLACE suffices for it.
DROP FUNCTION IF EXISTS post_ap_bill(UUID, CHAR, JSONB, DATE, UUID, UUID, TEXT);

-- Refresh comments that reference 'supplier' on the renamed objects.

COMMENT ON TABLE vendors IS
  'Vendor master data for the inflow cycle (Slice A). FK target for '
  'purchase_orders.vendor_id and vendor_bills.vendor_id. Currency is '
  'the vendor''s default billing currency; individual PO lines and '
  'bills carry their own currency.';

COMMENT ON INDEX accounts_ap_partitioned_uk IS
  'ap / ap_unsettled accounts partitioned by (counterparty_id, '
  'currency). Per-vendor liability tracking. Un-partitioned forms '
  '(counterparty_id NULL) remain valid for tests that don''t need '
  'per-vendor reporting.';

COMMENT ON INDEX accounts_vendor_pool_uk IS
  'vendor_pool qty-side accounts partitioned by counterparty_id. '
  'No currency on qty side. Per Slice A inflow design.';

-- ============================================================
-- post_po_receipt — refresh body with vendor terminology
-- ============================================================
...
COMMENT ON FUNCTION post_po_receipt(UUID, JSONB, DATE, UUID, UUID, TEXT) IS
  ...
  'DR / vendor_pool CR), value (inv_value_raw DR / ap_unsettled CR), '
...
-- ============================================================
-- post_ap_bill — rename p_supplier_id → p_vendor_id
-- ============================================================
