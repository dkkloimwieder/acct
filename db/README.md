...
- `bom_lines(bom_id, line_no, kind, basis, applies_at_op, fire_at, scrap_pct, component_sku_id, component_loc_id, qty_per_parent, absorption_class_id, std_amount)` — BOM contents. `kind` ∈ {`item`, `service`, `charge`}. `basis` ∈ {`per_unit`, `per_lot`}. `fire_at` ∈ {`wo_start`, `op_arrival`}. Discriminator CHECK enforces field combinations: items have component refs; services/charges have absorption_class + std_amount. Cross-table self-reference (component = parent) is caught by a row-level trigger raising **P0034** (`bom_line_self_reference`).
...
- `skus.yield_mode` ∈ {`plan_only`, `absorbed`} (default `plan_only`, mig 0061) — caller intent for the (future) BOM rollup tool. `plan_only`: `bom_lines.scrap_pct` is planning metadata only; rollup ignores it. `absorbed`: rollup inflates parent std by `1/(1-scrap_pct/100)` per item line. Pool flow (emission, op_move) is **literal** in both modes; the flag affects only the rollup tool's output.
