...
| `P0026` | `wo_invalid` | `post_wo_start` / `post_op_move` / `post_wo_complete` / `post_scrap` / `post_wo_close_unproduced` rejected the call: WO id not found, status not draft (for `post_wo_start`) or not released (for the others), `parent_sku.cost_method ≠ 'standard'` (Slice B MVP gate; lifted under `acct-p7v`), empty `wo_routings` for the WO, no `wo_outputs` rows on `post_wo_complete` (BOM2 path), qty ≤ 0, scrap qty exceeds `stock_wip` pool balance at the op, or a `bom_lines.absorption_class_id` whose `applied_account_kind` is unmapped by `_wo_apply_reason_for` (e.g. caller registered an absorption_class with `applied_account_kind='cogs'`). |
...
| `P0028` | `routing_op_invalid` | `post_op_move` got `from_op = to_op`, or `from_op` / `to_op` (or `post_scrap`'s `routing_op`) is not in this WO's `wo_routings`. Also raised by `post_wo_start` when a `bom_lines.applies_at_op` (post phantom explosion) doesn't match any of the WO's `wo_routings` rows — `wo_start_op_mismatch`. |
| `P0029` | `bom_missing` | **Deprecated** in BOM2 (acct-jtt, migration 0062). The pre-BOM2 path raised this when `boms` had zero rows for `parent_sku_id`. The new resolver `_wo_resolve_bom_for` raises **P0033** (`bom_header_resolution_invalid`) instead. The code is reserved (no current emitter) so historical alerts don't recycle. |
| `P0030` | `osp_yield_overflow` | Reserved. Originally for `post_osp_receive` when received qty > shipped qty; the `post_osp_ship`/`post_osp_receive` entry-points were rolled back via `acct-3uh` and will be rebuilt on standard ERP transactions when Slice C lands. |
| `P0031` | `eco_invalid_state` | `post_eco_approve` called on an ECO that is not in `'draft'` status, or on a draft ECO with a NULL `effective_at` / `requested_by`. The function flips status `draft → approved` only from a valid initial state; everything else is rejected as a caller bug. |
| `P0032` | `phantom_recursion_limit` | `_wo_explode_bom` exceeded the 16-level recursion depth, indicating either a runaway phantom chain or an outright cycle (`A → phantom B → phantom A`). Phantom expansion uses a recursive CTE with cycle detection; this code surfaces both conditions as one symptom for callers. |
| `P0033` | `bom_header_resolution_invalid` / `output_allocation_invalid` | Two compatible failure modes share this code: (a) `bom_header_at` / `_wo_resolve_bom_for` returned zero or multiple matches for `(parent_sku, alternate_no, business_date)` — caller pinned a `work_orders.bom_id` that doesn't exist or no primary active BOM is in effect at `business_date`; (b) `wo_outputs(wo)` rows sum to allocation_pct ≠ 100 (validated by `post_wo_start` and `post_wo_complete`). |
| `P0034` | `bom_line_self_reference` / `wo_close_unproduced_invalid_state` | (a) `bom_lines` insert / update where the line's `component_sku_id` equals the header's `parent_sku_id` (CHECK can't reference another table; this is a row-trigger fire). (b) `post_wo_close_unproduced` called against a WO that doesn't satisfy `qty_completed = 0 AND qty_scrapped = qty_target` — abandonment-close is only valid when nothing was produced and everything was scrapped. |
| `P0035` | `consumption_policy_not_implemented` | `post_wo_start` (or any `wo_events` insert with `event_kind='start'`) on a parent SKU whose `consumption_policy ≠ 'forward'`. The column exists for forward-compat with `acct-BACKFLUSH` Phase 2; the only dispatched policy today is `forward`. Enforced by a BEFORE INSERT trigger on `wo_events` (migration 0060). |
| `P0036` | `wac_periodic_pool_cycle` | `wac_periodic_close_hook`'s tier-2 topological pool walk (acct-smn, mig 0065) detected a cycle in the inv_value_wip pool DAG built from in-period op_move_v transfers. Rework loops (op_move 20→10) create the cycle; iterative-fixed-point handling deferred to acct-p7v-rework. The error message lists the unresolvable pool ids. Standard / wac_perpetual WOs doing rework are unaffected (their op_moves don't appear in transfers_provisional). |
...
### `post_wo_start` / `post_op_move` / `post_wo_complete` / `post_scrap` / `post_wo_close_unproduced` (Slice B + BOM2)

Slice B conversion cycle. Originally shipped against a flat `boms`+`wo_routing_burdens` model (migrations 0037–0039, acct-b82). The BOM2 refactor (acct-jg2, migrations 0040–0062) replaced that with a header/lines model + alternates + revisions + runtime-configurable absorption taxonomy. Migration 0062 (acct-jtt) retired the legacy tables; the function bodies described here all run the BOM2 path.

**Schema (BOM2; migrations 0037 base + 0040–0046 BOM2 Phase A)**:
- `work_orders(id, wo_no, parent_sku_id, fg_location_id, qty_target, qty_completed, qty_scrapped, status, currency, bom_id, posted_by, posted_at)` — header. status ∈ {`draft`, `released`, `closed`}. `bom_id` is the optional pin to a specific `bom_headers` row; NULL means "use primary active at business_date" via `_wo_resolve_bom_for`.
- `wo_routings(wo_id, routing_op, op_name)` — per-WO operation list (no shared template at MVP).
- `bom_headers(id, parent_sku_id, alternate_no, revision_no, code, description, status, is_primary, effective_at, obsolete_at, eco_id, created_at)` — BOM-as-a-thing. Status ∈ {draft, active, obsolete}. UNIQUE on `(parent_sku_id, alternate_no, revision_no)`. Partial UNIQUE INDEX `bom_headers_primary` enforces "one primary per (parent, alternate) at any time" while status='active'. CHECK on `effective_at < obsolete_at`. Time-phased revisions selected via `effective_at`/`obsolete_at` window.
- `bom_lines(bom_id, line_no, kind, basis, applies_at_op, fire_at, yield_pct, component_sku_id, component_loc_id, qty_per_parent, absorption_class_id, std_amount)` — BOM contents. `kind` ∈ {`item`, `service`, `charge`}. `basis` ∈ {`per_unit`, `per_lot`}. `fire_at` ∈ {`wo_start`, `op_arrival`}. `yield_pct` (NUMERIC(5,2), default 100) is per-line planned material yield — planning metadata only (renamed from `scrap_pct` in mig 0093). Discriminator CHECK enforces field combinations: items have component refs; services/charges have absorption_class + std_amount. Cross-table self-reference (component = parent) is caught by a row-level trigger raising **P0034** (`bom_line_self_reference`).
- `absorption_classes(id, code, display_name, applied_account_kind, expense_account_kind, ...)` — runtime burden taxonomy. The seed declares `labor_std → labor_applied` and `oh_std → oh_applied`. New classes (`osp_plating`, `production_setup`, `energy_hv`, …) are added via plain `INSERT` — no `ALTER TYPE` needed. New classes target `applied_account_kind='absorption_pool'` so they ride the generic `burden_apply` (per_unit) / `lot_charge_apply` (per_lot) reasons.
- `engineering_change_orders(id, code, status, requested_by, requested_at, approved_by, approved_at, effective_at, rejected_reason)` — ECO workflow scaffold. status ∈ {draft, approved, rejected, superseded}. Composite CHECK enforces `approved` requires `approved_by + approved_at + effective_at` and `rejected` requires `rejected_reason`.
- `wo_outputs(wo_id, output_no, output_sku_id, fg_location_id, qty, allocation_method, allocation_pct)` — co-product / by-product table. `allocation_method` ∈ {`primary`, `sales_value`, `fixed_ratio`, `market_price`}. `post_wo_start` auto-initializes a single-output `primary` row when empty; `post_wo_complete` distributes the FG drain across rows per `allocation_pct`.
- `wo_events(id, wo_id, event_kind, routing_op_from, routing_op_to, qty, business_date, posted_by, idempotency_key, notes)` — lifecycle audit log. event_kind ∈ {`start`, `op_move`, `wo_complete`, `scrap`}. BEFORE INSERT trigger gates `consumption_policy='forward'` only at WO start (P0035 otherwise).
- `skus.yield_mode` ∈ {`plan_only`, `absorbed`} (default `plan_only`, mig 0061) — caller intent for the (future) BOM rollup tool. `plan_only`: `bom_lines.yield_pct` is planning metadata only; rollup ignores it. `absorbed`: rollup inflates parent std by `1/(yield_pct/100)` per item line. Pool flow (emission, op_move) is **literal** in both modes; the flag affects only the rollup tool's output.
- `skus.default_lot_size` (default 1) — divisor for amortizing per-lot charges into the per-unit standard cost.
- `skus.is_phantom` (default FALSE) — phantom assembly flag. `_wo_explode_bom` flattens phantom child BOMs into the parent at the parent's `applies_at_op`.

**Cost rollup model (BOM2).** `parent_std_cost = Σ (item lines: qty_per_parent × component_std_cost) + Σ (per_unit service lines: std_amount) + Σ (per_lot lines: std_amount / default_lot_size)`. The rollup is a future tool; today the standard is set caller-side via `post_standard_cost_roll`. The `yield_mode` flag selects whether the rollup inflates by `1/(yield_pct/100)`.

**Helpers (BOM2 Phase B, migrations 0047–0051)**:

- `_wo_apply_reason_for(p_class_id BIGINT, p_basis TEXT) RETURNS transfer_reason` (mig 0047) — maps `(absorption_class, basis)` → reason. `labor_applied → labor_apply`; `oh_applied → oh_apply`; `absorption_pool + per_unit → burden_apply`; `absorption_pool + per_lot → lot_charge_apply`. Anything else raises **P0026**.
- `bom_header_at(p_parent_sku_id, p_alternate_no, p_business_date) RETURNS bom_headers` (mig 0048) — single-row resolver. **P0033** if zero or multiple matches.
- `_wo_resolve_bom_for(p_wo_id, p_business_date) RETURNS bom_headers` (mig 0049) — uses `work_orders.bom_id` if set; else delegates to `bom_header_at(parent, 1, business_date)` for the primary active BOM.
- `_wo_explode_bom(p_bom_id, p_business_date) RETURNS TABLE(...)` (mig 0050) — walks the BOM with recursive phantom expansion (16-level cap; **P0032** on cycle/depth).
- `_wo_emit_bom_lines(p_wo_id, p_bom_id, p_routing_op, p_qty, p_filter JSONB, p_event_id, p_business_date, p_posted_by) RETURNS JSONB` (mig 0051, rev 0061) — generic event-batch builder. `p_filter` selects by `{kind, basis, fire_at, applies_at_op}` subset. Items emit `rm_issue_to_wo` (qty + value); services/charges emit the canonical apply reason from `_wo_apply_reason_for`. Emission is **literal** — `bom_lines.yield_pct` is planning metadata only (acct-6jq).

**Document functions (BOM2; migrations 0052+0053+0055+0061+0062)**:

- `post_wo_start(p_wo_id, p_business_date, p_posted_by, p_idempotency_key, p_notes)` — releases a draft WO. Resolves BOM via `_wo_resolve_bom_for`, validates every `bom_lines.applies_at_op` (post phantom explosion) appears in `wo_routings` (**P0028** on mismatch), auto-inits `wo_outputs` if empty, emits `wo_start` qty leg + `fire_at='wo_start'` lines (any applies_at_op) + `fire_at='op_arrival', applies_at_op=first_op` lines, all charging WIP@first_op. Flips status `draft → released`.
- `post_op_move(p_wo_id, p_from_op, p_to_op, p_qty, …)` — moves qty units between ops, then applies destination-op lines. Value-leg amount = `qty × std_cum_at_from_op` where `std_cum = per_unit_cum (items + per_unit services at applies_at_op ≤ from_op) + per_lot_cum (lines fired by now / default_lot_size)`. Reason `op_move_v` bypasses the dispatcher's standard branch (which would return parent's full std cost — wrong at intermediate ops). **First arrival** at `p_to_op` emits per_unit + per_lot lines at `p_to_op`; **subsequent arrival** (rework) emits per_unit *services* only — items fire once per lifecycle (at wo_start or first arrival of `applies_at_op`) so rework doesn't double-issue components (acct-jtt fix).
- `post_wo_complete(p_wo_id, p_qty, …)` — completes qty units from the highest routing_op. Validates `wo_outputs.allocation_pct` sums to 100 (**P0033**), reads pool@last_op `FOR UPDATE`, **pre-balances** pool vs `parent_std × qty` via a `wo_close_v` event (favorable when pool < drain → DR pool / CR variance; unfavorable when pool > drain → DR variance / CR pool). The pre-balance step makes both variance directions reachable under standard cost-on-WIP; pre-acct-6jq the favorable case hit the debit-normal CHECK. Distributes the drain across `wo_outputs` (qty leg `wo_complete`, value leg `wo_complete_v` to bypass dispatcher pricing), then on final close walks **all** `inv_value_wip(parent, op)` accounts and absorbs any non-last-op residual into `variance_wo_close` (op_move integer truncation, etc.). Status → `closed`.
- `post_scrap(p_wo_id, p_routing_op, p_qty, …)` — unchanged from Slice B. Reads `inv_value_wip` + `stock_wip@op` `FOR UPDATE`, computes accumulated unit cost, emits `scrap` qty leg + `scrap_v` value leg routed through `variance_scrap`.
- `post_wo_close_unproduced(p_wo_id, p_business_date, p_posted_by, p_idempotency_key)` (mig 0056, B10) — closes a fully-scrapped WO without a `post_wo_complete`. Validates `qty_completed = 0 AND qty_scrapped = qty_target` (**P0034** otherwise), walks all `inv_value_wip(parent, op)` for the WO and absorbs residuals into `variance_wo_close`, sets status `closed`.
- `post_eco_approve(p_eco_id, p_approved_by, p_approved_at, p_effective_at)` (mig 0058, B12) — workflow function for approving an `engineering_change_orders` row. Flips status `draft → approved`, populates the BOM headers tied to the ECO with `effective_at`, optionally obsoletes predecessor revisions. **P0031** if the ECO is not in `'draft'` status or required fields are missing.

**MVP gate.** WO `parent_sku.cost_method = 'standard'` (P0006 otherwise; lifted under acct-p7v which adds WAC to WIP). Components may use any cost method but **must** have a `standard_costs` row in effect at `business_date` (`resolve_standard_cost_at` raises P0018 otherwise).

**Idempotency.** Each function checks `wo_events.idempotency_key` on entry (fast path) and again after the `FOR UPDATE` on `work_orders` (race-safe — fixes the wo_start / wo_complete status-transition window in migration 0039). Replays return `p_wo_id` without re-emitting events.

**WIP locking + read-then-write.** `post_scrap` and `post_wo_complete`'s pre-balance + residual-sweep paths acquire `FOR UPDATE` on `inv_value_wip` before reading `(debits_total - credits_total)` — the read-then-write under lock pattern allowed in v0.2. Same-tx FOR UPDATE re-acquisition by the subsequent `post_transfers` call is a no-op upgrade.

**Errors.** P0026 (wo_invalid: not found, wrong status, parent ≠ standard, no routing, no wo_outputs on complete, scrap > pool, unmapped absorption_class); P0027 (qty_overflow); P0028 (routing_op_invalid: from=to, op not in routing, applies_at_op mismatch); P0032 (phantom recursion); P0033 (BOM resolution / output allocation); P0034 (bom_line self-reference, close_unproduced state); P0035 (consumption_policy not 'forward'); P0010 if any required account isn't open; P0018 if a component lacks a standard cost row; P0001–P0005 inherit from `post_transfers`.
...
**Out of scope until Phase 1:** `customers`, `users`, `routings`, `facilities`. `vendors` shipped in Slice A (acct-7mg, migration 0034) — `purchase_orders.vendor_id` and `vendor_bills.vendor_id` now have real FKs. `sales_orders.customer_id` is still a nullable UUID with no FK — the `customers` table doesn't exist yet. `work_orders` + `wo_routings` shipped in Slice B (acct-h7j, migration 0037); `bom_headers` + `bom_lines` + `absorption_classes` + `engineering_change_orders` + `wo_outputs` shipped in BOM2 (acct-jg2, migrations 0040–0046).
