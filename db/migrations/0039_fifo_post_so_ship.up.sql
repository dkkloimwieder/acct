-- ============================================================
-- Phase E1 follow-up W4 (acct-clrd) part 3/3.
--
-- Drop FIFO from post_so_ship's IN ('fifo','lot') P0006 gate and
-- add a dedicated FIFO branch for v_unit_cost (audit field on
-- so_shipment_lines.unit_cost). The FIFO branch:
--
--   1. Locks v_val_acct (inv_value_fg) FOR UPDATE — same lock the
--      WAC branch acquires for AP9/R7 audit-field consistency.
--   2. Walks _fifo_walk_layers under that lock; sums cost_amount
--      into v_walk_total.
--   3. Sets v_unit_cost := v_walk_total / v_qty_shipped (BIGINT
--      integer division — weighted average; per-layer truth lives
--      in cost_layer_depletions.unit_cost).
--
-- Caller-supplied amount on the value-leg is v_qty_shipped *
-- v_unit_cost. The dispatcher's compute_amount path then walks
-- layers AGAIN under the same lock via _compute_amount_fifo_outbound
-- and OVERRIDES the caller's amount with SUM(cost_amount) — exact
-- per-layer-ROUND total. apply_event re-walks a third time for
-- depletion writeback (mig 0037 widens the E1 gate to inv_value_fg
-- so this fires). All three walks see identical allocations because
-- they share the same FOR UPDATE locks within the same transaction.
--
...
BEGIN
  ...
  FOR v_idx IN 1..v_n LOOP
    ...
    ELSIF v_cost_method = 'fifo' THEN
      -- W4: walk FG layers under FOR UPDATE on v_val_acct (mirrors
      -- the WAC branch's lock pattern for AP9/R7 audit consistency).
      -- Dispatcher walks again under the same lock for the actual
      -- ledger amount; apply_event re-walks for depletion writeback.
