-- ============================================================
-- Phase E1 follow-up W2 (acct-8skl, sub-issue of acct-xxrs).
--
-- Unblocks FIFO via post_inventory_adjustment. Splits FIFO out of
-- the IN ('fifo','lot') P0006 gate into its own WHEN branch.
--
-- +qty (adjustment-in): caller MUST supply p_unit_cost. The
--   value-leg posts inv_value_raw DR; apply_event's E1 block
--   creates a cost_layers row at p_unit_cost.
--
-- -qty (adjustment-out): caller MUST NOT supply p_unit_cost.
--   The wrapper walks _fifo_walk_layers under FOR UPDATE to compute
--   the total amount, then emits the value-leg with that walked
--   total as the caller-supplied amount. Reason='inventory_adjustment'
--   is not in the dispatcher's cost-event list so the amount stands.
--   apply_event's E1 block re-walks layers under the same locks (same
--   txn) and writes per-layer cost_layer_depletions whose cost_amounts
--   sum to posting_line.amount exactly (per-layer ROUND construction).
--
-- v_effective_uc on adjustment-out is the weighted average
-- (walked_total / abs(qty_delta)). It's a BIGINT audit field on
-- inventory_adjustments.unit_cost; per-layer truth lives in
-- cost_layer_depletions.unit_cost (NUMERIC).
--
-- MVP scope restriction: FIFO adjustment supports inventory_class='raw'
-- only. FG-FIFO and WIP-FIFO are out of scope; raises P0006 with a
-- pointer to the W4 follow-up (FG-FIFO via post_so_ship).
--
...
BEGIN
  ...
  WHEN 'fifo' THEN
    -- W2: FIFO at MVP supports inv_value_raw layers only. FG-FIFO
    -- and WIP-FIFO are out of scope (W4 / Phase E1 follow-up).
    ...
    PERFORM 1 FROM accounts WHERE id = v_lock_second FOR UPDATE;
    IF p_qty_delta > 0 THEN
      -- Adjustment-in: caller MUST supply unit_cost; apply_event's
      -- E1 block creates a cost_layers row at this unit_cost.
      IF p_unit_cost IS NULL THEN
        RAISE EXCEPTION
      ...
      v_effective_uc := p_unit_cost;
    ELSE
      -- Adjustment-out: caller MUST NOT supply unit_cost. Walk
      -- layers under FOR UPDATE for the total; emit value-leg with
      -- the walked total. apply_event re-walks under the same locks
      -- (same txn) for depletion writeback — sums match by per-layer
      -- ROUND construction.
      IF p_unit_cost IS NOT NULL THEN
        RAISE EXCEPTION
      ...
      -- Audit-field weighted average. May not equal any individual
      -- layer's unit_cost; per-layer truth lives in cost_layer_depletions.
  ...
  -- FIFO -qty branch already set v_val_amount via the layer walk.
  -- All other branches leave it NULL through CASE; recompute from
  -- v_qty_amount × v_effective_uc here.
