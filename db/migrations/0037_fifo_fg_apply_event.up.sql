-- ============================================================
-- Phase E1 follow-up W4 (acct-clrd) part 1/3.
--
-- Extend apply_event's E1 block from inv_value_raw only to
-- (inv_value_raw, inv_value_fg) on both DR (receipt-side layer
-- creation) and CR (issue-side depletion writeback) sides. This
-- unblocks FG-FIFO at the apply layer:
--
--   * Receipt-side: post_wo_complete drains WIP → FG. Debit is
--     inv_value_fg with parent_sku.cost_method='fifo'. Now the
--     E1 block creates one cost_layers row at v_total_drain/p_qty
--     unit_cost.
--   * Issue-side: post_so_ship's cogs leg credits inv_value_fg
--     with FIFO SKU. Now the E1 block walks layers via
--     _fifo_write_depletions and stamps cost_layer_id.
--
-- Body otherwise identical to mig 0032's _post_posting_lines_apply_event.
-- 'lot' still raises P0006 elsewhere (gates haven't moved).
-- ============================================================
...
BEGIN
  ...
  IF v_qty_for_row IS NOT NULL
    ...
    --
    -- W4 (acct-clrd): receipt and issue gates extended from
    -- 'inv_value_raw' to ('inv_value_raw', 'inv_value_fg') so
    -- post_wo_complete creates an FG layer (DR inv_value_fg with
    -- FIFO parent) and post_so_ship walks FG layers (CR inv_value_fg
    -- with FIFO SKU).
