-- ============================================================
-- Phase E2 E2.4 — lot subledger residual recon (acct-ujqw,
-- sub-issue of acct-uze).
--
-- Adds two checks to run_daily_reconciliation, mirroring the
-- FIFO check #8 precedent (mig 0033, acct-swwp):
--
--   Check 9 'lot_residual_mismatch' — per (product, location)
--     lot_total   = SUM(inventory_lots.original_quantity)
--     event_total = SUM(inventory_lot_events.quantity_change)
--                   (signed; transfer/hold/release/status_change
--                   contribute 0 by CHECK)
--     residual    = lot_total + event_total
--     on_hand     = stock_available debits - credits, summed across
--                   lot partitions for the (sku, location)
--     Alert when  residual <> on_hand
--
--     Filter: only inspect (product, location) where the SKU's
--     cost_method = 'lot_fifo'. Mirrors FIFO check #8's filter.
--
--   Check 10 'lot_negative_residual' — per-lot defensive sentinel
--     residual_per_lot = original_quantity
--                      + COALESCE(SUM(quantity_change), 0) over
--                        events sharing (lot_id, lot_receipt_date)
--     Alert when residual_per_lot < 0
--
--     Should never fire in production (wrappers prevent over-
--     consumption via FOR UPDATE walks). Catches direct-INSERT
--     bypass / dispatcher bugs.
--
-- Both checks emit alerts via INSERT INTO reconciliation_alerts
-- (alert_kind, payload). reconciliation_alerts.alert_kind is TEXT
-- (mig 0020) — no enum change required.
--
-- The function body is the FULL run_daily_reconciliation (checks
-- 1-8 verbatim from mig 0033) plus the two new checks appended
-- before RETURN. CREATE OR REPLACE; previous body is the
-- mig 0033 form.
-- ============================================================
...
BEGIN
  ...
  --
  -- Per (product, location) for lot_fifo SKUs, the lot subledger
  -- residual must equal the stock_available net qty (summed across
  -- per-lot partitions of stock_available, since accounts.lot_id
  -- partitions stock_available for lot SKUs per mig 0044).
  --
  -- Lot side aggregates across all lots at (product, location):
  --   residual = SUM(original_quantity) over lots
  --            + SUM(quantity_change) over events of those lots
  -- Account side:
  --   on_hand  = SUM(debits_total - credits_total) over all
  --              stock_available partitions for that (sku, location)
  --
  -- Both sides cast to NUMERIC (lots are NUMERIC(19,6); accounts
  -- are BIGINT). Exact equality required, mirroring check 8.
  INSERT INTO reconciliation_alerts (alert_kind, payload)
  WITH lot_aggr AS (
  ...
  --
  -- Defensive: residual_per_lot = original_quantity + SUM(events).
  -- Wrappers + dispatcher prevent over-consumption via FOR UPDATE
  -- walks; this check catches direct INSERT bypass or future
  -- dispatcher bugs. Should never fire in production.
  INSERT INTO reconciliation_alerts (alert_kind, payload)
