-- acct-5ba / BOM2 Phase B5 — _wo_emit_bom_lines generic event-batch emitter.
--
-- Walks _wo_explode_bom(p_bom_id, p_business_date), filters by p_filter,
-- and produces a JSONB array of transfer events suitable for post_transfers.
--
-- Filter (any subset; missing keys are wildcards):
--   {kind: 'item'|'service'|'charge',
--    basis: 'per_unit'|'per_lot',
--    fire_at: 'wo_start'|'op_arrival',
--    applies_at_op: <int>}
--
-- All emitted events charge p_routing_op's WIP value account. (Caller
-- decides which WIP op to charge — first_op for wo_start lines and the
-- arrival op for op_arrival lines.)
--
-- Event shapes:
--   item:           qty leg + value leg (rm_issue_to_wo)
--                   adj_qty = CEIL(p_qty × qty_per_parent / (1 - scrap_pct/100))
--                   value = adj_qty × resolve_standard_cost_at(comp_sku, business_date)
--   service per_unit  service per_lot  charge per_lot:
--                   single value-only leg (debit WIP, credit applied/pool)
--                   per_unit amount = p_qty × std_amount
--                   per_lot  amount = std_amount
--                   reason via _wo_apply_reason_for(class_id, basis)
--
-- Errors:
--   P0010 missing account (stock_consumed / stock_available / inv_value_raw /
--         inv_value_wip / applied class account).
--   P0026 wo_invalid (WO not found, qty <= 0, absorption_class missing).
--   Forwarded: P0029 (orphan phantom) / P0032 (recursion limit) from
--   _wo_explode_bom.
...
BEGIN
  ...
  -- Parent's WIP value account at the caller-specified op.
  ...
  -- Parse filter (NULL keys = wildcard).
  ...
  LOOP
    IF v_line.kind = 'item' THEN
      -- qty calculation with optional scrap_pct gross-up.
      IF v_line.scrap_pct = 0 THEN
        v_adj_qty := p_qty * v_line.qty_per_parent;
      ELSE
        v_adj_qty := CEIL(
          (p_qty * v_line.qty_per_parent)::NUMERIC / (1 - v_line.scrap_pct / 100.0)
        )::BIGINT;
      END IF;
...
COMMENT ON FUNCTION _wo_emit_bom_lines(UUID, BIGINT, INT, BIGINT, JSONB, UUID, DATE, UUID) IS
  'Generic BOM-line emitter. Walks _wo_explode_bom, filters by '
  '{kind, basis, fire_at, applies_at_op} subset, emits a JSONB array of '
  'rm_issue_to_wo and absorption-apply events charging WIP at p_routing_op. '
  'Per-unit lines scale by p_qty; per-lot lines fire once at std_amount. '
  'Item lines apply scrap_pct gross-up via CEIL(p_qty × qpp / (1-pct/100)). '
  'acct-5ba.';
