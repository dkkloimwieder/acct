...
--
-- CONTEXT. mig 0028/0071 blocks post_standard_cost_roll with P0006
-- ('wip_present_blocks_std_cost_roll') if any open inv_value_wip pool
-- exists for the SKU. In high-volume / continuous-production shops
-- WIP is never zero, so the roll is operationally unreachable.
--
-- This migration adds an opt-in p_revalue_wip BOOLEAN parameter. When
-- TRUE the roll skips the gate and, after the existing raw/fg
-- revaluation loop, walks the SKU's open inv_value_wip pools posting
-- pool_qty × (p_new_cost − v_prior) against a new account_kind
-- 'variance_wip_revaluation'.
--
-- WHY pool_qty × Δstd is correct (no chronological replay needed).
-- The bd description models WIP material as `qty × prior_standard at
-- issue time`. Under the invariant 'every roll revalues WIP
-- atomically' (which this migration enforces by tying WIP revaluation
-- to the same transactional unit as the standard write), all parents
-- currently in WIP have their material slice valued at the most
-- recent prior standard. After this roll, all WIP material is at
-- p_new_cost. Subsequent rolls again see one-and-only-one prior. So
-- the formulation pool_qty × Δstd matches mig 0071's raw/fg shape;
-- the labor / OH / absorption-class portion of inv_value_wip.balance
-- is untouched because we ADD a delta calculated from qty against
-- variance_wip_revaluation — we don't multiply pool_value by a ratio.
--
-- WHY pool_qty comes from stock_wip (not transfers.qty on the value
-- pool). For inv_value_wip pools, transfers.qty is mixed: rm_issue_
-- to_wo's qty is component qty, op_move_v's qty is parent qty.
-- Summing them gives a meaningless number. The paired stock_wip
-- account is per-(sku, location, routing_op) — its balance is parent
-- qty currently in WIP at that op. Mirrors mig 0064's
-- _wac_close_pool_qty_in dispatch.
--
-- LIMITATION. Assumes parent_std_at_issue = Σ(component_std ×
-- qty_per_parent), i.e. parent is homogeneous-standard with standard
-- components. If the parent has wac_* components, rm_issue_to_wo
-- posted the value-leg at the component running avg (acct-rgb tier
-- 1/2/3 dispatch), so material in WIP isn't strictly tied to
-- parent_std × parent_qty. Real ERPs operating in standard-cost mode
-- accept this approximation; the gap is documented in the bd issue
-- and tracked as a follow-up if Phase 1 surfaces production pressure.
--
-- INTEGRATION SHAPE. p_revalue_wip is added to post_standard_cost_
-- roll directly (not as a separate post_wip_material_revaluation
-- function). Atomicity is the point: the roll and the WIP
-- revaluation must succeed or fail together so the no-mixed-prior
-- invariant holds. Decoupling them would let a caller roll without
-- revaluing, breaking the invariant on the next roll.
--
-- Callers using the old 8-arg signature still work — the default is
-- FALSE which preserves prior behavior bit-for-bit.

ALTER TYPE account_kind ADD VALUE IF NOT EXISTS 'variance_wip_revaluation';

-- DROP+CREATE because adding a parameter changes the function
-- signature (PG forbids CREATE OR REPLACE across signature changes).
...
BEGIN
  ...
  -- WIP gate. acct-bru lifts when p_revalue_wip = TRUE — caller opts
  -- into the companion revaluation that keeps the no-mixed-prior
  -- invariant. p_revalue_wip = FALSE preserves mig 0028/0071
  -- behavior bit-for-bit (gate trips, no WIP loop).
  ...
  IF NOT v_future_dated AND v_prior IS NOT NULL AND v_prior <> p_new_cost THEN
    -- Lock raw/fg AND wip value pools (when revaluing WIP) ascending
    -- by id to preserve the lock order discipline. wip stock_wip qty
    -- accounts are read but not mutated; FOR UPDATE on the value
    -- pool serializes with op_move_v / wo_complete_v / scrap_v
    -- callers that would mutate qty.
    ...
    -- routes through variance_wip_revaluation — distinct P&L line
    -- from variance_std_cost_roll so income statement reports
    -- pre-shipment WIP-side revaluation separately from raw/fg-side.
    -- Bidirectional (debit-normal pool grows on positive delta;
    -- credit-side bound by inv_value_wip CHECK preventing negative
    -- balance, which doesn't fire here because we ADD against a
    -- positive-balance pool — Δstd direction matches the pool's debit
    -- normal).
    IF p_revalue_wip THEN
      -- inv_value_wip pools key on (sku, routing_op); the paired
      -- stock_wip account also keys on (sku, routing_op) only —
      -- location is not part of the WIP key (mirrors mig 0021's
      -- _post_transfers_lookup_qty_account dispatch).
      FOR v_wip_record IN
        SELECT v.id           AS val_acct,
               v.currency     AS currency,
...
COMMENT ON FUNCTION post_standard_cost_roll(
  UUID, BIGINT, DATE, DATE, UUID, UUID, TEXT, BIGINT, BOOLEAN
) IS
  'standard_cost_roll workflow (Phase 1 Epic F, mig 0028; per-class '
  'qty divisor mig 0071 / acct-du2.11; WIP material revaluation '
  'companion mig 0078 / acct-bru). p_revalue_wip=TRUE skips the '
  'wip_present_blocks_std_cost_roll gate and walks open inv_value_wip '
  'pools posting pool_qty × Δstd against variance_wip_revaluation '
  '(parent qty read from the paired stock_wip account, mirroring '
  '_wac_close_pool_qty_in dispatch). Material slice grows by Δstd × '
  'parent_qty; absorbed labor / OH / burden value untouched.';
