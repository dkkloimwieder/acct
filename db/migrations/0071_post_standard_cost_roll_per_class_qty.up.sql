-- acct-du2.11 — post_standard_cost_roll uses per-class qty for revaluation.
--
-- BUG. mig 0028 reads v_pool_qty from stock_available(sku, location) inside
-- the per-pool revaluation loop, then applies the same v_pool_qty to BOTH
-- inv_value_raw AND inv_value_fg pools at the same location.
--
-- Example: SKU has raw_qty=30 + fg_qty=20 at MAIN; stock_available qty
-- is the cross-class sum 50. Roll 100 → 130 (delta=30/unit):
--
--   raw pool delta = 50 × 30 = 1500   (should be 30 × 30 = 900)
--   fg  pool delta = 50 × 30 = 1500   (should be 20 × 30 = 600)
--
-- Each class is over-revalued by the OTHER class's qty share. Same shape
-- as the original acct-fii bug in post_cost_adjustment (fixed in mig
-- 0069). Sibling site overlooked at the time.
--
-- FIX. Drop the JOIN to stock_available; compute v_pool_qty as the
-- per-class signed SUM on transfers.qty filtered to v.id (the value
-- account itself). Mirrors the acct-fii pattern in mig 0069 and the
-- per-class divisor pattern in mig 0030 (acct-1vr).
--
-- LOCK ORDER preserved: the loop iterates v_lock_ids ascending; only
-- inv_value_* pools are locked (stock_available was never in the lock
-- set, only read). Removing the read also dissolves acct-du2.12 (AP3
-- on stock_available read without FOR UPDATE) — there's no read to
-- protect.
--
-- BEHAVIOR-PRESERVING for the single-class case (raw OR fg, not both)
-- because per-class signed SUM on transfers.qty equals stock_available
-- qty when only one class has activity. Bug surfaces only in the
-- multi-class same-location regime, undetected until tests/standard_cost_
-- roll.rs is extended (this migration adds the test in tests/, not
-- here).
...
BEGIN
  ...
  IF NOT v_future_dated AND v_prior IS NOT NULL AND v_prior <> p_new_cost THEN
    ...
    -- Walk each value pool. acct-du2.11 fix: per-class signed SUM on
    -- transfers.qty filtered to v.id (the value account itself), NOT
    -- stock_available qty (which is cross-class). Mirrors mig 0069
    -- post_cost_adjustment fix shape. The JOIN to stock_available is
    -- removed — qty no longer needs to come from there.
...
COMMENT ON FUNCTION post_standard_cost_roll(
  ...
  'standard_cost_roll workflow (Phase 1 Epic G, mig 0028). acct-du2.11 '
  '(mig 0071): per-pool revaluation qty divisor reads per-class signed '
  'SUM on transfers.qty filtered to the value account, not '
  'stock_available cross-class qty. Class-isolated when raw + fg share '
  'a location; previously over-revalued each class by the other class''s '
  'qty share.';
