-- acct-69p (review of acct-b82) — fix idempotency replay race in
-- post_wo_start and post_wo_complete.
--
-- ============================================================
-- The bug
-- ============================================================
--
-- post_wo_start and post_wo_complete each transition work_orders.status
-- (draft → released; released → closed on final completion). The
-- original implementation in migration 0038 placed the wo_events
-- idempotency-replay check BEFORE the FOR UPDATE on work_orders. With
-- two concurrent calls T1, T2 carrying the same idempotency_key:
--
--   T1: replay-check (not found) → FOR UPDATE → status='draft' → ...
--       INSERT wo_events → post_transfers → UPDATE status='released'
--       → COMMIT (releases lock)
--   T2: replay-check (not found yet, T1 hadn't committed) → waits on
--       FOR UPDATE → T1 commits → T2 acquires lock → reads status =
--       'released' (T1's write is visible) → status check raises P0026
--
-- A user re-issuing the same idempotency_key should get a no-op
-- replay, not P0026. The race window is narrow but real (any time
-- two clients call with the same key while T1 is mid-flight).
--
-- post_op_move and post_scrap don't have this issue: status doesn't
-- transition during the call, so the second caller's status check
-- still passes after acquiring the lock, and the INSERT INTO wo_events
-- ... ON CONFLICT DO NOTHING handles the actual race-resolution.
--
-- ============================================================
-- The fix
-- ============================================================
--
-- Add a second replay check immediately AFTER the FOR UPDATE on
-- work_orders, before the status check. After the lock is held, any
-- concurrent T1 has either (a) not yet inserted its wo_events row
-- (we'll proceed normally and our INSERT...ON CONFLICT will catch any
-- last-microsecond race), or (b) already committed and we'll see its
-- wo_events row, returning idempotently.
--
-- The first (fast-path, no-lock) replay check is retained — it's the
-- common case and avoids the FOR UPDATE wait when replaying after T1
-- has committed.
--
-- This is a function-only migration; no schema change.

-- ============================================================
-- post_wo_start (replay-check moved/duplicated)
-- ============================================================
...
BEGIN
  ...
  -- Lock work_orders row to serialize concurrent callers.
  ...
  -- Race-safe replay check AFTER lock. If a concurrent T1 finished
  -- while we were waiting for FOR UPDATE, its wo_events row is now
  -- visible — return idempotently instead of raising P0026 on the
  -- now-released status.
  SELECT id INTO v_existing_id FROM wo_events
   WHERE idempotency_key = p_idempotency_key;
  IF v_existing_id IS NOT NULL THEN RETURN p_wo_id; END IF;
...
-- ============================================================
-- post_wo_complete (replay-check moved/duplicated)
-- ============================================================
