-- ============================================================
-- acct-sxl2.6 — Recon check #14: lot_unit_count_mismatch.
--
-- WHAT:
--   CREATE OR REPLACE run_daily_reconciliation, copied verbatim
--   from mig 0060 (post-sxl2.5 baseline is unchanged) plus new
--   check #14 inserted after check #13.
--
-- WHY:
--   sxl2.5 closed the wrapper layer for lot_and_serial: every
--   lifecycle transition that touches the lot residual now also
--   touches inventory_units (receipt creates units; rm_issue +
--   so_ship + inventory_adjustment -qty flip them to terminal
--   statuses; lot_transfer moves them to the dest lot). The
--   recon check #14 holds the two grain levels accountable to
--   each other.
--
-- HOW:
--   Per (lot_id, lot_receipt_date) on tracked_by='lot_and_serial'
--   SKUs, COUNT(inventory_units WHERE status IN active) must
--   equal the lot's residual qty (mirror of check #9 but at unit
--   grain). Drift surfaces:
--     (a) phantom unit  — unit row at status=available but parent
--         lot fully drained (wrapper forgot to flip the unit's
--         status when lot event was emitted),
--     (b) phantom residual — lot has residual but no active units
--         (wrapper forgot to create unit at receipt, or unit was
--         hand-deleted bypassing the wrapper).
--
-- DESIGN CALLS:
--   - Filter LEFT JOIN to inventory_units anchored on the lot side
--     so we catch BOTH directions in one pass.
--   - Use the same "active statuses" tuple as the partial UNIQUE
--     on inventory_units (mig 0061): available / reserved /
--     allocated / on_hold / returned. Terminal statuses (shipped /
--     consumed / transferred_out / scrapped / lost) correctly
--     drop out of the count — they should match a fully-drained
--     lot residual.
--   - Skip lots whose SKU is tracked_by != 'lot_and_serial' (no
--     units exist for those by design).
--   - Equality check: residual <> COALESCE(units, 0). No
--     tolerance (units are integer counts).
-- ============================================================
