-- acct-du2.3 — post_osp_ship / post_osp_receive idempotency dual-check.
--
-- BUG. mig 0057 lines 55-57 (post_osp_ship) and 157-159 (post_osp_receive)
-- pre-check transfers.idempotency_key, then take FOR UPDATE on
-- work_orders. A concurrent T1 with the same key can commit a transfer
-- between T2's pre-check and T2's PERFORM post_transfers (line 118 / 221).
-- T2's post_transfers INSERT trips the UNIQUE constraint on
-- transfers.idempotency_key, raising an error rather than returning
-- idempotently — same shape as the acct-69p bug fixed for post_wo_start
-- in mig 0039.
--
-- FIX. Re-check transfers.idempotency_key immediately AFTER the FOR
-- UPDATE on work_orders; bail out if found. Mirrors mig 0039's
-- dual-check pattern.
...
BEGIN
  -- Fast-path replay check (pre-FOR UPDATE).
  ...
  -- Race-safe replay re-check after lock (acct-du2.3 / acct-69p shape).
  -- A concurrent caller with the same key may have committed its
  -- transfer between the pre-check and our FOR UPDATE acquisition.
  SELECT id INTO v_existing FROM transfers
   WHERE idempotency_key = p_idempotency_key;
  IF v_existing IS NOT NULL THEN RETURN p_wo_id; END IF;
