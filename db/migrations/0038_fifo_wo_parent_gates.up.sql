-- ============================================================
-- Phase E1 follow-up W4 (acct-clrd) part 2/3.
--
-- Widen parent-SKU cost_method gates in post_wo_start, post_op_move,
-- post_wo_complete to admit 'fifo'. For value dispatch, FIFO parent
-- is treated identically to WAC parent (running avg from WIP pool):
-- WIP-FIFO is out of scope, so the WIP pool acts as a single pool
-- during the WO. The drain at wo_complete debits inv_value_fg → mig
-- 0037's E1 block creates the FG layer at v_total_drain/p_qty
-- unit_cost.
--
-- Bodies are identical to mig 0017 except:
--   - post_wo_start: gate at line 1189 includes 'fifo'.
--   - post_op_move: WAC dispatch branch at line 1478 includes 'fifo'.
--   - post_wo_complete: WAC dispatch branch at line 1742 includes 'fifo'.
--
-- By-products pre-pass at post_wo_complete remains standard-only
-- (FIFO parent + by-products is acct-nnyl follow-up territory).
-- ============================================================
...
BEGIN
  ...
  ELSIF v_cost_method IN ('wac_perpetual', 'wac_periodic',
                          'wac_retroactive', 'fifo') THEN
    -- W4: FIFO parent treated as WAC parent for op_move value calc
    -- (WIP-FIFO out of scope; WIP pool acts as a single pool).
...
BEGIN
  ...
  ELSIF v_cost_method IN ('wac_perpetual', 'wac_periodic',
                          'wac_retroactive', 'fifo') THEN
    -- W4: FIFO parent treated as WAC parent for value drain
    -- computation. The drain debits inv_value_fg (per output) and
    -- mig 0037's E1 block creates an FG layer at v_v_share/v_q_share
    -- per-output. Subsequent post_so_ship walks FG layers FIFO.
  ...
  -- By-products pre-pass on closing call. Standard parent only
  -- (acct-nnyl follow-up for non-standard parent + by-product
  -- interaction; FIFO parent included in that future scope).
