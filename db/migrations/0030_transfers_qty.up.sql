-- acct-1vr / acct-75z.1 — Per-class qty tracking on transfers.
--
-- Fixes the structural coupling bug between qty-side and value-side
-- pools that breaks per-class WAC math when a single SKU has
-- multi-state inventory simultaneously (raw + fg open).
--
-- ROOT CAUSE: _post_transfers_lookup_qty_account (migration 0021,
-- still active) maps both inv_value_raw and inv_value_fg to the SAME
-- stock_available(sku, location) qty account. Correct accounting
-- (raw and fg are both "available" by qty) but breaks per-class cost
-- division when a SKU has multiple value pools active. Manifestations:
--
--   1. wac_periodic_close_hook (0029): final period avg = numerator-
--      per-class / denominator-pooled-across-classes -> wrong.
--
--   2. wac_perpetual compute_amount (0029): unit cost =
--      inv_value_<pool>.balance / stock_available.balance -> divisor
--      inflates if multi-class. Currently latent (no test exercises
--      multi-class on wac_perpetual).
--
-- FIX: persist per-event qty on transfers as a new column. The new
-- column lets each cost-method computation derive a class-isolated
-- qty divisor by summing transfers tagged with debit/credit on the
-- specific value pool — no longer goes through the pooled
-- stock_available balance.
--
-- ALL CHANGES IN ONE MIGRATION (tightly coupled):
--   - schema: ALTER TABLE transfers ADD COLUMN qty BIGINT (nullable;
--             pre-0030 historical rows have qty IS NULL; SUMs ignore
--             NULL).
--   - _post_transfers_compute_amount: wac_perpetual + wac_periodic
--     branches use new per-class SUM as divisor.
--   - post_transfers (single-pass + two-pass): populate qty at
--     INSERT time per the source rule below.
--   - post_inventory_adjustment: pass qty in both leg JSON objects.
--   - wac_periodic_close_hook: qty_in computation reads
--     transfers.qty directly instead of going through stock_available.
--
-- _post_transfers_lookup_qty_account is RETAINED — narrowed scope.
-- It's still needed by post_transfers' lock pre-scan to add the qty
-- account to the FOR UPDATE set (correctness for the qty-leg event's
-- own UPDATE on stock_available, not for divisor computation).
--
-- QTY SOURCE RULE (in post_transfers INSERT):
--   1. If event has 'qty' JSONB field → qty := (p_event->>'qty')::BIGINT
--      (caller-asserted; cost-relevant value-leg events always supply this).
--   2. Else if both sides have ledger_kind='qty' (qty-leg event where
--      amount IS qty by convention) → qty := amount.
--   3. Else qty := NULL (cash, AR, AP, FX, and other non-inventory).
--
-- post_inventory_adjustment populates 'qty' on BOTH legs of its v_batch
-- so the rule sees it on both qty-leg and value-leg events.
--
-- FORWARD-ONLY. Pre-0030 transfers stay qty=NULL forever. SUMs ignore
-- NULL. Phase 0/1 has no closed wac_periodic period to "miss"; dev DB
-- is wiped via ./scripts/dev-down.sh --wipe; test DB recreates per run.
-- Down-migration drops the column (data loss is acceptable).

-- ============================================================
-- Schema
-- ============================================================

ALTER TABLE transfers ADD COLUMN qty BIGINT;

ALTER TABLE transfers
  ADD CONSTRAINT transfers_qty_nonneg
  CHECK (qty IS NULL OR qty >= 0);

COMMENT ON COLUMN transfers.qty IS
  'Per-event physical quantity. Populated for inventory-touching events '
  '(both qty-side legs and value-side cost-relevant). NULL for non-'
  'inventory transfers (cash, AR, AP, FX) and pre-0030 historical rows. '
  'Forward-only from migration 0030 (acct-1vr).';

COMMENT ON FUNCTION _post_transfers_lookup_qty_account(accounts) IS
  'Maps a value-side account to its matching qty-side account. As of '
  'acct-1vr (migration 0030), only used by post_transfers'' lock '
  'pre-scan to add the qty account to the FOR UPDATE set — divisor '
  'computation in WAC branches now reads transfers.qty directly.';

-- ============================================================
-- _post_transfers_compute_amount: wac_perpetual + wac_periodic
-- branches use per-class SUM(transfers.qty) as divisor.
-- ============================================================
...
BEGIN
  ...
  CASE p_cost_method
    ...
    WHEN 'wac_perpetual' THEN
      ...
      -- Per-class qty: net qty currently in this value pool, computed
      -- from class-tagged transfer history (signed by direction).
      -- Replaces the old stock_available.balance divisor which pooled
      -- raw and fg qty for the same (sku, location).
    ...
    WHEN 'wac_periodic' THEN
      -- D6 from acct-3e1: WIP pools out of scope for Phase 1 wac_periodic.
...
-- ============================================================
-- post_transfers: add qty resolution + populate at INSERT.
-- Single-pass and two-pass paths each get the new column.
-- ============================================================
...
BEGIN
  ...
  IF NOT v_has_wac THEN
    ...
    FOR v_idx IN 1..v_n LOOP
      ...
      -- Resolve qty for the new row per the source rule:
      --   1. Caller-asserted (event has 'qty' field) wins.
      --   2. Else qty := amount when both sides are qty-class.
      --   3. Else NULL.
      v_qty_for_row := (v_event->>'qty')::BIGINT;
      IF v_qty_for_row IS NULL
         AND v_d_acct.ledger_kind = 'qty' AND v_c_acct.ledger_kind = 'qty' THEN
      ...
      INSERT INTO transfers (
        ...
        v_d_id, v_c_id, v_amount, v_qty_for_row,
...
-- ============================================================
-- post_inventory_adjustment: populate qty on both legs of v_batch.
-- wac_perpetual + wac_periodic OUT branches now use per-class SUM
-- for pool avg computation (consistent with compute_amount).
-- ============================================================
...
BEGIN
  ...
  -- Early WIP-class guard for wac_periodic (D6).
  ...
  WHEN 'wac_perpetual' THEN
    ...
    -- Per-class qty for this value pool (signed sum from history).
  ...
  -- Build batch. Both legs carry 'qty' so post_transfers persists it.
...
-- ============================================================
-- wac_periodic_close_hook: qty_in reads transfers.qty directly.
-- ============================================================
...
BEGIN
  ...
  LOOP
    ...
    -- value_in: per-class debits to this pool during the period (numerator).
    ...
    -- qty_in: per-class qty entering this pool during the period
    -- (denominator). Reads transfers.qty directly — no longer goes
    -- through the pooled stock_available account.
    SELECT COALESCE(SUM(t.qty), 0) INTO v_pool_qty_in
      FROM transfers t
     WHERE t.debit_account_id = v_pool_acct.id
