//! T1 probes for `posting_lines_provisional` (migration 0025). Audit
...
async fn one_transfer(pool: &PgPool) -> i64 {
    ...
    call_post_posting_lines(pool, json!([event]), false).await.expect("seed transfer");
        ...
        sqlx::query(
            "INSERT INTO posting_lines_provisional
                (posting_line_id, period_id, cost_method, finalized_at,
                 variance_amount, variance_posting_line_id)
...
async fn finalized_zero_variance_with_transfer_id_violates_check() {
    ...
    // finalized + variance=0 must have variance_posting_line_id NULL.
    expect_sqlstate("23514", || async {
        sqlx::query(
            "INSERT INTO posting_lines_provisional
                (posting_line_id, period_id, cost_method, finalized_at,
...
async fn finalized_nonzero_variance_without_transfer_id_is_allowed() {
    ...
    // variance_posting_line_id NULL. The CHECK was relaxed to permit this.
    let pool = connect_test_db().await;
    reset_to_fixture(&pool).await;
    ...
    .execute(&pool)
    .await
    .expect("non-zero variance with NULL posting_line_id allowed post-tier-2");
}
...
async fn variance_transfer_id_self_reference_violates_check() {
    ...
    // variance_posting_line_id == posting_line_id is rejected (would self-loop).
    expect_sqlstate("23514", || async {
        sqlx::query(
            "INSERT INTO posting_lines_provisional
                (posting_line_id, period_id, cost_method, finalized_at,
...
async fn unknown_transfer_id_fk_violation() {
    ...
    // posting_line_id pointing to a non-existent transfer → FK violation
    // (23503).
...
async fn duplicate_transfer_id_violates_pk() {
    ...
    // PK violation — posting_line_id is the PK of posting_lines_provisional, so
