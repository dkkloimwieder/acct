...
**For Phase 1 regression detection:** L's 5-min runs land in a 2 800-2 900 evps band when the DB is fully warm; short-run sampling (60 s) shows the central tendency is closer to ~2 300 evps with ±15 % noise on this rig (see Caveats). Don't read a single short-run drop as a regression — re-sample first. A genuine architectural regression would be a sustained median drop below ~2 000 evps across multiple short-run samples, OR a clear drop in the 5-min steady-state band.
...
- **Shape L is unchanged by `acct-uxu`.** An initial 3-run × 5-min comparison after `acct-uxu` (migration 0021) suggested a ~16 % regression (2 876 → 2 428 evps median). A follow-up under proper noise rejection — 5 samples × 60 s with 30 s gaps between samples, on the same hardware on 2026-04-30 — found pre-WAC median = 2 247 evps and post-WAC median = 2 381 evps with completely overlapping ranges (both [≈2 050-2 200, 2 508]) and per-distribution spreads of 13.8 % – 20.4 %. Post-WAC is statistically indistinguishable from pre-WAC; the original 16 % "regression" was within noise. Shape L's headline number (2 876 evps) is best read as the upper observed end of L's distribution; the central tendency on this rig is closer to ~2 300-2 400 evps. **No hot-path optimization needed for `acct-uxu`'s WAC dispatcher.** (Filed and closed as `acct-ezm`.)
